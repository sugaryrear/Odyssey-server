...
public class TourneyManager {
    ...
    private static final int ARENA_X = 3557, ARENA_Y = 6112;
    ...
    public static boolean handleObjects(Player player, WorldObject object, int optionId) {
      //  System.out.println(singleton.getTimeLeft());
        if (object == null || object.getId() != 39640)
            return false;
        ...
        switch (optionId) {
            ...
            case 3:
                player.sendMessage(TourneyManager.getSingleton().isLobbyOpen() ? "The tournament lobby is open and will start shortly." : TourneyManager.getSingleton().isArenaActive() ? "The Lone Surviver tournament has already begun." : "The next Last Man Standing will begin in: " + TourneyManager.getSingleton().getTimeRemaining());
                return true;
        }
        return false;
    }
    ...
    private void beginTournament() {
        ...
        for (String p : currentPlayers) {
            Player player = PlayerHandler.getPlayerByDisplayName(p);
            if (player == null) {
                toRemove.add(p);
                continue;
            }
    ...
    public void join(Player player) {
      ...
      //  player.getPA().movePlayer(new Coordinate(3141, 3641));
        PathFinder.getPathFinder().findRouteNoclip(player, player.absX, 3640, true, 0, 0);
    ...
    private void updateInterface(boolean updatePlayers) {
        currentPlayers.forEach(p -> {
            ...
            if (player != null) {
           player.sendMessage(isLobbyOpen() ? getLobbyTime() :  hasTarget(player) ? getFogTime(player) : "Waiting for target..");
            player.getPA().sendFrame126(isLobbyOpen() ? getLobbyTime() :  hasTarget(player) ? getFogTime(player) : "Waiting for target..", 266);
                if (updatePlayers)
                  player.sendMessage(isLobbyOpen() ? "Current Players In Lobby: @or1@" + currentPlayers.size() : "Players Left In Arena: @or1@" + currentPlayers.size());
    ...
    public void leaveLobby(Player player, boolean xlog) {
        if (!player.spectatingTournament && isParticipant(player)) {
            ...
            if (xlog)
                player.getPA().movePlayer(new Coordinate(3141, 3636));
    ...
    public void handleKill(Entity player) {
        ...
        if (player instanceof Player) {
            ...
            killer.setRunEnergy(10000, true);
    ...
    public void handleDeath(String playerName, boolean logout) {
       ...
       //     player.getPA().forceMove(3141, 3634, 0, logout);
            player.getPA().movePlayer(new Coordinate(3141, 3634));
        }
        ...
        for (int i = victors.size() - 1; i >= 0; i--) {
            ...
            if (player != null) {
                ...
                if (player.getMode().getCoinRewardsFromTournaments()) {
                    if (i == 2) {
                        player.getItems().addItemToBankOrDrop(995, (1000 * tier) + (100));//1st place gets 6m + 1m every 2 people
                    } else {
                        player.getItems().addItemToBankOrDrop(995, (500 * tier) + (400));//2nd and 3rd get 2m + 500k ever 2 people
                    }
                    ...
                    if (i == 2) {
                        player.getItems().addItemToBankOrDrop(995, (10000 * tier) + (100));//1st place gets 6m + 1m every 2 people
                    } else if (i != 2) {
                        player.getItems().addItemToBankOrDrop(995, (500 * tier) + (400));//2nd and 3rd get 2m + 500k ever 2 people
                    }
                    player.tournamentPoints += 1; //winner of tournament gets  a tournament win
                ...
                if (i == 2) {
                    player.streak += 1;
                    if (player.streak == 5) {
                        player.sendMessage("@red@You receive an extra 50k coins for having a 5 win streak.");
                        announce("@blu@" + name + " has just reached 5 wins in a row in Lone Surviver!");
                        player.getItems().addItemToBankOrDrop(995, (50000));
                        player.attacking.reset();
                    } else if (player.streak == 10) {
                        player.sendMessage("@red@You receive an extra 100kcoins for having a 10 win streak.");
    ...
    public boolean isInLobbyBounds(Player player) {
        //3138, 3639, 3144, 3645
        return player.absX >= 3138 && player.absX <= 3144 && player.absY >= 3639 && player.absY <= 3645;
    }
