...
public class ChangeDisplayName {
    ...
    public static void clickChangeName(Player player) {
...
//        }
    ...
    private static void start(Player player) {
        ...
        new DialogueBuilder(player).setNpcId(Npcs.ACCOUNT_GUIDE)
                .npc("This will allow you to set your display name.", "@blu@You will continue to login with '@pur@" + player.getLoginName() + "@blu@'.")
                .npc("@red@WARNING@bla@: when logging in, donating, and voting", "use @blu@"+player.getLoginName()+"@bla@")
                .npc("Your display name is what everyone will see your name as.")
                .option("Change display name?",
                        new DialogueOption("Change display name", plr -> correct.send()),
                        new DialogueOption("Cancel", plr -> incorrect.send()))
                .send();
    }
...
private static void trySetDisplayName(Player player, String name) {
    ...
    private static boolean isInvalid(Player player, String name) {
        boolean valid = RS2LoginProtocol.checkUsername(name) == LoginReturnCode.SUCCESS && !PlayerSave.playerExists(name);
...
//                    .npc(DialogueExpression.HAPPY, "Your display name was set to '" + name + "'.")
//                    .send();
//            // plr.getItems().deleteItem(Items.NAME_CHANGE_SCROLL, 1);
//
//            if (plr.clan != null)
//                plr.clan.updateDisplayName(player);
//            plr.updatePlayerPanel();
//
