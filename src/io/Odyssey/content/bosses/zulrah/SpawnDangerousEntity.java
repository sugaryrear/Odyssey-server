		...
		if (ticks == 4 || ticks == 8 || ticks == 12 || ticks == 16) {
			for (Point point : location.getPoints()) {
				if (entity.equals(DangerousEntity.TOXIC_SMOKE)) {
					Server.getGlobalObjects().add(new GlobalObject(Zulrah.TOXIC_SMOKE, point.x, point.y, zulrah.getInstancedZulrah().getHeight(), 0, 10)
							.setInstance(zulrah.getInstancedZulrah()));
					CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
						@Override
						public void execute(CycleEventContainer container) {
//							if(player == null){
//								Server.getGlobalObjects().remove(Zulrah.TOXIC_SMOKE,zulrah.getInstancedZulrah());
//								container.stop();
//							}
							Server.getGlobalObjects().remove(Zulrah.TOXIC_SMOKE,zulrah.getInstancedZulrah());
							container.stop();
