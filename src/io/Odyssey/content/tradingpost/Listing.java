package io.Odyssey.content.tradingpost;
...
import io.Odyssey.util.logging.player.TradingPostBuyLog;
...
public class Listing {
	...
	public static void buyListing(Player c, int slot, int amount) {
		...
		if(PlayerHandler.getPlayerID(sales.getName()) != -1) {
			...
			DiscordMessager.sendLogs("[TRADING POST] " + c.getDisplayName() + " bought " + formatItemName(sales.getId()) + " x" + amount + " from " + seller.getDisplayName());
			if(seller != null) {
		...
		switch(button) {
			case 189237:
				...
				if(c.amDonated <= 49 && total >= 6) {
					...
					return;
				} else if(c.amDonated >= 25 && c.amDonated <= 99 && total >= 7) { //regular donator
					...
					return;
				} else if(c.amDonated >= 250 && c.amDonated <= 999 && total >= 10) { //diamond club
