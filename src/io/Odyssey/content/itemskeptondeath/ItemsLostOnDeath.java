...
public class ItemsLostOnDeath {
    ...
    public static int getKeptItemAmount(Player player) {
        ...
        if (player.isSkulled)
            amount -= 3;
        if (CombatPrayer.isPrayerOn(player, CombatPrayer.PROTECT_ITEM) || CombatPrayer.isPrayerOn(player, CombatPrayer.CURSE_PROTECT_ITEM) )
            amount += 1;
        return amount;
    }
    ...
    public static ItemsLostOnDeathList generateModified(Player player) {
        ...
        for (GameItem lostItem : new ArrayList<>(list.getLost())) {//modify always lost items here
