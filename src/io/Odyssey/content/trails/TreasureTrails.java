...
import io.Odyssey.content.skills.hunter.impling.ItemRarity;
...
public class TreasureTrails {
	...
	public static final int TEAL_AOD_CLUESCROLL = 30106;

	public static final int TEAL_AOD_CASKET= 30107;

	public static final int RED_AOD_CASKET= 30110;
	...
	private static int getCluesCompleted(Player player, RewardLevel difficulty) {
		switch (difficulty) {
			...
			case TEAL:
				return player.getTealAodChestCounter();
			case RED:
				return player.getRedAodChestCounter();
		}
		return 0;
	}
	...
	private static void openCasket(Player player, RewardLevel rewardLevel) {
		...
		System.out.println("called opencasket "+rewardLevel == null ? "null":"not null");
		...
		switch (rewardLevel) {
			...
			case MASTER:
				...
				PetHandler.roll(player, PetHandler.Pets.BLOODHOUND); // broken
				break;
			case TEAL:
				player.setTealAodChestCounter(player.getTealAodChestCounter() + 1);
				player.sendMessage("<col=2d256d>You have completed " + player.getTealAodChestCounter() + " Angel Of Death Rare Rewards Chest.");
				break;
			case RED:
				player.setRedAodChestCounter(player.getRedAodChestCounter() + 1);
				player.sendMessage("<col=2d256d>You have completed " + player.getRedAodChestCounter() + " Angel Of Death Rare Rewards Chest.");
				break;
		}
	}
