...
public class OptionDialogue extends DialogueObject {
    ...
    public void send(Player player) {
        ...
        if (farSwords) {
            player.getPA().sendInterfaceHidden(1, firstChildId);
            player.getPA().sendInterfaceHidden(0, secondChildId);
        } else {
            player.getPA().sendInterfaceHidden(0, firstChildId);
            player.getPA().sendInterfaceHidden(1, secondChildId);
        }
