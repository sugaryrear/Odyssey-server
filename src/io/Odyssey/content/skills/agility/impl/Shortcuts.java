...
import static io.Odyssey.model.Items.MITH_GRAPPLE;
...
public class Shortcuts {
	...
	public boolean agilityShortcuts(final Player c, final int objectId) {
		...
		switch (objectId) {
			case 26404://lvl 60 wildy shortcut to godwars
			case 26405:
				if (c.playerLevel[c.playerAgility] < 60) {
					c.sendMessage("You must have an agility level of at least 60 to climb this.");
					return false;
				}

				if (c.getAgilityHandler().hotSpot(c, 2950,3767))
					c.getAgilityHandler().move(c, -8, 0, 844, -1);
				if (c.getAgilityHandler().hotSpot(c, 2942,3768))
					c.getAgilityHandler().move(c, 8, 0, 844, -1);
				break;
			case 26400://shortcut to godwars
			case 26402:

				if (c.getAgilityHandler().hotSpot(c, 2928,3760))
					c.getAgilityHandler().move(c, 0, -4, 844, -1);
				if (c.getAgilityHandler().hotSpot(c, 2928,3757))
					c.getAgilityHandler().move(c, 0, 4, 844, -1);
				break;

			case 154:
				if(c.playerLevel[Player.playerAgility] < 70) {
					c.sendMessage("You need have an Agility level of at least 80 to jump here.");
					return false;
				}
			...
			case 30189:
				...
				if (c.objectX == 2881 && c.objectY == 9825) {
				c.getPA().movePlayer(2880,9825,1);
					} else 	if (c.objectX  == 2904 && c.objectY == 9813) {
					c.getPA().movePlayer(2902,9813,1);
				}

				//
				return true;
			case 30191://ladder down to slayer tower basement
				if (c.getSlayer().getTask().isPresent()) {
					c.getPA().movePlayer(3412, 9932, 3);
				} else {
					c.sendMessage("You must have an existing task to go down to the basement.");
				}
				break;
			case 28849:
				if (c.playerLevel[Player.playerAgility] < 63) {
					c.sendMessage("You need an agility level of at least 63 to squeeze through here.");
					return false;
				}
		...
		case 4469:
					...
					return false;
						...
						CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
							...
							public void execute(CycleEventContainer container) {
								...
								if (c.getAgilityHandler().hotSpot(c, 2878, 9813)) {
									c.setForceMovement(2880, 9813, 0, 100, "EAST", 2750);//jump over

									CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
										@Override
										public void execute(CycleEventContainer container) {
											c.getPA().addSkillXPMultiplied(12.5, Skill.AGILITY.getId(), true);
											c.getDiaryManager().getFaladorDiary().progress(FaladorDiaryEntry.JUMP_FLOOR_SPIKES);

											container.stop();
										}
									}, 2);
								} else 	if (c.getAgilityHandler().hotSpot(c, 2880, 9813)) {
									c.setForceMovement(2878, 9813, 0, 100, "WEST", 2750);//jump over
			...
			case 16509:
				...
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						//if (finalDoor_totheright != null) {
						if (c.getAgilityHandler().hotSpot(c, 2886, 9799)) {
							c.setForceMovement(2892, 9799, 0, 100, "EAST", 747);//crawl thru

						} else 	if (c.getAgilityHandler().hotSpot(c, 2892, 9799)) {
							c.setForceMovement(2886, 9799, 0, 100, "WEST", 747);//balance
...
//				}
//				if (c.getAgilityHandler().hotSpot(c, 2886, 9799)) {
//					c.setForceMovement(2886, 9799, 0, 100, "WEST", 762);//balance
//
//				}
...
//			return true;
		...
		case 8729: // Wyvern top floor
			...
			if (!c.getSlayer().getTask().get().getPrimaryName().equals("skeletal wyvern")) {
				c.sendMessage("You must have an active skeletal wyvern task to enter this crevice.");
				return false;
			}
			...
			case 30170:
				...
				AgilityHandler.delayEmote(c, "CRAWL", 1918, c.absY > 4366 ? 4365 : 4367, 0, 2);
				return true;
		case 10596:
		...
		case 10595:
			...
			return true;
			case 17062://catherby grapple to tavelry
				if (c.playerLevel[Player.playerAgility] < 36) {
					c.sendMessage("You need an agility level of at least 36 to cross here.");
					return false;
				}
				if (c.playerLevel[Player.playerStrength] < 22) {
					c.sendMessage("You need a strength level of at least 22 to cross here.");
					return false;
				}
				if (c.playerLevel[Player.playerRanged] < 29) {
					c.sendMessage("You need a ranged level of at least 29 to cross here.");
					return false;
				}
				if (!c.getItems().playerHasItem(MITH_GRAPPLE)) {
					c.sendMessage("You need a mithril grapple to cross here.");
					return false;
				}
				...
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						//if (finalDoor_totheright != null) {
						if (c.absY < 3429) {
							c.getPA().movePlayer(2840,3435,0);
						} else  {
							c.getPA().movePlayer(2840,3426,0);
		...
		case RELLEKKA_CREVICE:
			...
			case SLAYER_TOWER_CHAIN_UP:
				if (c.getAgilityHandler().getLevelRequired(SLAYER_TOWER_CHAIN_UP) > c.playerLevel[Player.playerAgility]) {//or else it spams
					c.sendMessage("You need an agility level of at least " + c.getAgilityHandler().getLevelRequired(objectId) + " to do this.");
					return false;
				}
				if (z == 0 && y <= 3552) {
