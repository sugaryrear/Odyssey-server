...
public class Firemaking {
	...
	public static void lightFire(final Player player, final int logUsed, final String usage) {
		...
		if (log == null) {
			LightSources.lightSource(player,TINDERBOX,logUsed);
			...
			if (usage == "tinderbox") {
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						GlobalObject fire = new GlobalObject(5249, coords[0], coords[1], player.heightLevel, 0, 10);
		...
		Server.getGlobalObjects().remove(fire);
					Server.getGlobalObjects().updateObject(fire, -1);
