...
public class Farming {
		...
		if (inhabited(patch.bottomLeft.getX(), patch.bottomLeft.getY())) {
			...
			for (Plant plant : plants) {
				if (plant != null && plant.getPatch() == patch) {
					System.out.println("plant: "+plant.getConfig());
	...
	public void doConfig() {
		...
		switch (closest) {
			case FALADOR_HERB:
			//	System.out.println("closest here");
...
//Config value: configValue = (configValues[0] << 16) + (configValues[1] << 8 << 16) + configValues[2] + (configValues[3] << 8);
				...
				//int configValue = configValues[i] | (configValues[i + 1] << 8);
				break;
			case CATHERBY_HERB:
				/**
				 * //		529(<<0) = Fruit Tree
				...
				//(configValues[i] << 8) | configValues[i + 1] ;
				//System.out.println("config: "+config+"  here");
				break;
			case ARDOUGNE_HERB:
