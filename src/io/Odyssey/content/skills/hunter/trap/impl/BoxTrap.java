...
public final class BoxTrap extends Trap {
	...
	public void onCatch(NPC npc) {
		...
		event = Optional.of(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				NPCDumbPathFinder.walkTowards(npc, getObject().getX(), getObject().getY());//ya this is how u make the npc go somewhere. dont forget randomwalk = false before!!
				if(isAbandoned()) {
					container.stop();
					npc.randomWalk = true;
					return;
				...
				if(npc.absX == getObject().getX() && npc.absY == getObject().getY()) {
					...
					if(!Server.getGlobalObjects().anyExists(getObject().getX(), getObject().getY(), getObject().getHeight())) {
						System.out.println("Hunter; No trap existed while attempting to catch2");
						return;
					}
					...
					if(count > formula) {
						//here where it happens when npc walks over it but it "Fails"
						setState(TrapState.FALLEN);
						npc.forceChat("Can't catch me!");
						npc.randomWalk = false;
						container.stop();
						return;
					}
	...
	public enum BoxTrapData {
		FERRET(1505, 1, 115, 10, new GameItem(10092, 1)),
		CHINCHOMPA(2910, 1, 198.25, 12, new GameItem(10033, 1)),
		CARNIVOROUS_CHINCHOMPA(2911, 63, 265, 14, new GameItem(10034, 1)),
		BLACK_CHINCHOMPA(2912, 73, 335, 25, new GameItem (11959, 1));
