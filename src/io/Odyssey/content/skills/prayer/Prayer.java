...
public class Prayer {
	...
	public void alter(final int amount, int objectX, int objectY) {
		...
		if (Boundary.isIn(player, Boundary.WILDY_BONE_ALTAR)) {
			player.getPA().addSkillXPMultiplied(bone.getExperience() * 3.5, Skill.PRAYER.getId(), true);
		} else {
			player.getPA().addSkillXPMultiplied(bone.getExperience(), Skill.PRAYER.getId(), true);
		}
