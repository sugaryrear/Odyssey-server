...
public class Slayer {
	...
	public static int[] SUPERIOR_COUNTERPARTS = { 448, 406, 414, 7272, 421, 419, 435, 417, 446, 484, 7276, 437, 7277, 3209, 6, 7279, 423, 411, 498, 1543, 4005, 415, 11,11 };
	public boolean superiorSpawned;

	public static final int EASY_TASK_NPC_ID = 401;
	public static final int MEDIUM_TASK_NPC_ID = 402;

	public static final int VANNAKA_TASK_NPC_ID = 403;
	public static final int CHAELDAR_TASK_NPC_ID = 404;
...
//					}
			...
			task = Optional.of(Misc.randomSearch(available, 0, exclusiveLength));//funny way of getting random data from an array
//			if (Server.isDebug()) {
...
//			}
			...
			if (masterId == 8623) {
				...
				player.talkingNpc = masterId;
				player.lastTask = task.get().getFormattedName();
				player.getDH().sendNpcChat("Excellent, you're doing great. Your new task is to kill", taskAmount+" "+task.get().getFormattedName());
				player.nextChat = -1;
			}
		});
	}
public boolean canigetatask(Task task) {
	//Task task = player.getSlayer().getTask().get();
		return false;
}
	/**
	...
	public boolean hasSlayerHelmBoost(NPC npc, CombatType combatType) {
		if (npc != null && (isTaskNpc(npc) || npc.getNpcId() == Npcs.MAX_DUMMY)) {
			...
			IntPredicate hasHelmet = id -> player.getItems().isWearingItem(id, Player.playerHat);//interesting IntPredicate
...
//		if(task.isPresent())
//		System.out.println("name: "+name+" "+task.get().getPrimaryName());
//		if(ArrayUtils.contains(task.get().getNames(), name)){
//			System.out.println("yes it does");
//		}
		return task.isPresent() && name.contains(task.get().getPrimaryName()) || task.isPresent() && ArrayUtils.contains(task.get().getNames(), name);
	}
...
//								if (npc.getNpcId() == 494) {
//									player.getPA().addSkillXPMultiplied(npc.getNpcStats().getHitpoints(), Skill.SLAYER.getId(), true);
//								} else {
									player.getPA().addSkillXPMultiplied(player.getRechargeItems().hasAnyItem(13113, 13114, 13115) && Boundary.isIn(player, Boundary.SLAYER_TOWER_BOUNDARY) ? (int) (npc.getNpcStats().getHitpoints() * 1.10) : npc.getNpcStats().getHitpoints(),
...
//						this.points += (m.getPointReward(0) * multiplier);
						...
						player.sendMessage("You have completed " + consecutiveTasks + " tasks in a row!");
...
//						}
						if(m.getId() != 401) {
							...
							if (consecutiveTasks == 10) {
								...
								points += (m.getPointReward(1) * multiplier);

								player.sendMessage("@blu@You have completed " + consecutive + " tasks in a row and receive additional points.");

							} else if (consecutiveTasks == 50) {

								points += (m.getPointReward(2) * multiplier);
								player.sendMessage("@blu@You have completed " + consecutive + " tasks in a row and receive additional points.");

							} else if (consecutiveTasks == 100) {

								points += (m.getPointReward(3) * multiplier);
								player.sendMessage("@blu@You have completed " + consecutive + " tasks in a row and receive additional points.");

							} else if (consecutiveTasks == 250) {
								points += (m.getPointReward(4) * multiplier);
								player.sendMessage("@blu@You have completed " + consecutive + " tasks in a row and receive additional points.");

							} else if (consecutiveTasks == 1000) {
								points += (m.getPointReward(5) * multiplier);
								player.sendMessage("@blu@You have completed " + consecutive + " tasks in a row and receive additional points.");
...
//							}
	 ...
	 */
	private Task[] retainObtainable(Task[] tasks) {//creates what tasks you can potentiall get based on your slayer level
		...
		if (unlocks.contains(unlock)) {
			...
			switch (unlock) {
				case CURSES:
						points -= cost;
					unlocks.add(unlock);
					player.getPA().sendFrame70(0, 0, 127_890);
					player.getPA().sendFrame70(0, 0, 127_891);
					player.getPA().sendFrame70(0, 0, 5608);
					player.getPA().sendFrame70(0, 0, 687);
					player.getPA().sendFrame70(0, 0, 5651);
					player.sendMessage("You purchase the ability to use @pur@Ancient Curses@bla@.");
					return true;
...
//			updatePoints();
	...
	public void teletoMaster(int masterId){
		switch(masterId){
			case 0:
				//player.sendMessage("You do not have a slayer master assigned.");
				break;
			case 401:
				player.getPA().startTeleport(2048,3583, 0, "modern", false);
				//player.getPA().startTeleport(3106, 3502, 0, "modern", false);
				break;
			case 404:
				player.getPA().startTeleport(2048,3583, 0, "modern", false);
				break;
			...
			case 405:
				player.getPA().startTeleport(2048,3583, 0, "modern", false);
			//	player.getPA().startTeleport(3507, 3503, 0, "modern", false);
				break;
			case 403:
				player.getPA().startTeleport(2048,3583, 0, "modern", false);
				//player.getPA().startTeleport(3147, 9913, 0, "modern", false);
	...
	public int getTaskAmountorig() {
		return taskAmountorig;
	}
	/**
	...
	public void setTaskAmountorig(int taskAmount) {
		this.taskAmountorig = taskAmount;
	}
