 ...
 */
public class CoinBagEmpty extends CycleEvent {

    /**
     * The item id of the PvM Casket required to trigger the event
     */
    public static final int MYSTERY_BOX = 10831; //Casket
     ...
     */
    public CoinBagEmpty(Player player) {
        this.player = player;
    }
    ...
    public void openall() {
        ...
        int coins = 250 + Misc.random(1000);
        int amount = player.getItems().getItemAmount(10831);
        ...
        if (Misc.random(1000) == 0) {
            ...
            player.getItems().deleteItem(10831, amount);
        } else {
            int rewardAllGpAmount = (coins);
            player.getItems().deleteItem(10831, amount);
            player.getItems().addItem(995, rewardAllGpAmount);
