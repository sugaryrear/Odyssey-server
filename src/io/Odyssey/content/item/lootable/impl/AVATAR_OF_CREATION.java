...
public class AVATAR_OF_CREATION implements Lootable {

    private static final int KEY = AvatarOfCreation.KEY;
    ...
    static {
        items.put(LootRarity.COMMON, Arrays.asList(
                //UPDATE AvatarOfCreationCHESTITEMS TOO
    ...
    private static GameItem randomChestRewardsCommon() {
        ...
        List<GameItem> itemList = avatarOfCreationChestItems.getItems().get(LootRarity.COMMON);
        return Misc.getRandomItem(itemList);
    }

    private static GameItem randomChestRewardsRare() {
        List<GameItem> itemList = avatarOfCreationChestItems.getItems().get(LootRarity.RARE);
        return Misc.getRandomItem(itemList);
    }
        ...
        if (random < rareChance) {
            ...
            if (c.getItems().playerHasItem(KEY)) {
                ...
                if (reward.getId() != Items.TOME_OF_FIRE_EMPTY) {
                    ...
                    PlayerHandler.executeGlobalMessage("@bla@[<col=3d7018>AvatarOfCreation@bla@] The AvatarOfCreation chest rewarded a <col=47831c>rare seed!");
                }
            } else {
                c.sendMessage("@blu@The chest is locked, it won't budge!");
            }
