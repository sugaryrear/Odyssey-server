...
import io.Odyssey.content.bosses.GiantMole;
import io.Odyssey.content.bosses.Vorkath;
import io.Odyssey.content.item.lootable.impl.RaidsChestItems;
...
public class CollectionLog {
	 ...
	 * Opens the interface for a player
	 */
	public void cltopen(Player player) {
		player.setViewingCollectionLog(this);
	}
	/**
	...
	public int totalitems_thatyouhave(Player player) {
		int	totalitems = selectTab_letscount_whatwegot(player, CollectionTabType.BOSSES) + selectTab_letscount_whatwegot(player, CollectionTabType.WILDERNESS) +
				selectTab_letscount_whatwegot(player, CollectionTabType.RAIDS) +  selectTab_letscount_whatwegot(player, CollectionTabType.MINIGAMES ) +  selectTab_letscount_whatwegot(player, CollectionTabType.OTHER);
		return totalitems;
	}
	public int totalitems_tocollect(Player player) {
		int	totalitems = selectTab_letscount(player, CollectionTabType.BOSSES) + selectTab_letscount(player, CollectionTabType.WILDERNESS) +
				selectTab_letscount(player, CollectionTabType.RAIDS) +  selectTab_letscount(player, CollectionTabType.MINIGAMES ) +  selectTab_letscount(player, CollectionTabType.OTHER);
		return totalitems;
	}
	public int selectTab_letscount(Player player, CollectionTabType type) {
		int amt = 0;
...
//							drops = RaidsChestRare.getRareDrops();
	...
	public int selectTab_letscount_whatwegot(Player player, CollectionTabType type) {
		int amt = 0;
...
//							drops = RaidsChestRare.getRareDrops();
	...
	public void selectTab(Player player, CollectionTabType type) {
		...
		if (npcs != null) {
			...
			for(int i = 0; i < npcs.size(); i++) {
				...
				if (getCollections().containsKey(npcs.get(i) + "")) {
					ArrayList<GameItem> itemsObtained = getCollections().get(npcs.get(i) + "");
					//	System.out.println("npc: "+npcs.get(i)+" and items: "+itemsObtained.size());
					if (itemsObtained != null) {
						List<GameItem> drops = Server.getDropManager().getNPCdrops(npcs.get(i));
						if (npcs.get(i) == 8028) {
							drops = Vorkath.getVeryRareDrops();
						...
						if (npcs.get(i) == 7554) {
							drops = RaidsChestItems.getRareDrops();
						} else if (npcs.get(i) >= 1 && npcs.get(i) <= 4) {
							drops = TreasureTrailsRewardItem.toGameItems(TreasureTrailsRewards.getRewardsForType(npcs.get(i)));
						} else if (npcs.get(i) == PETS_ID) {
							drops = PetHandler.getPetIds(true);
							...
							drops = TombsOfAmascutChest.getRareDrops();
						}else if (npcs.get(i) == GIANT_MOLE) {
							drops = GiantMole.getRareDrops();
						}else if (npcs.get(i) == 19990) {//pest control cheaphax
							drops = PestControl.getRareDrops();
						}
						//		System.out.println("drops: "+drops.size()+" and obtained: "+itemsObtained.size());
	...
	public void populateInterface(Player player, int npcId) {
		...
		if (npcId == GIANT_MOLE) {
			player.dropItems = GiantMole.getRareDrops();
		}
		if (npcId == 19900) {
			player.dropItems = GiantMole.getRareDrops();
			//	player.getPA().sendFrame126(Misc.optimizeText(npcName) + ": @whi@" + player.tobCompletions, 23120);
