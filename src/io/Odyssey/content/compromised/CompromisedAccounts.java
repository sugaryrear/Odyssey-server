package io.Odyssey.content.compromised;
...
public class CompromisedAccounts {
    ...
    public static void init() {
        try {
            if (!Server.isPublic())
                return;
            File file = new File(Server.getSaveDirectory() + "Odyssey_Compromised_Accounts.json");
            if (!file.exists()) {
    ...
    private static boolean message(Player player) {
        ...
        if (type != null) {
            ...
            String[] message = {
                    "Although Odyssey has not been compromised, it has been determined",
