...
//here its YOU attacking the npc pov
    public boolean hasDistanceAndPathToAttack(Entity entity, boolean followingLogic) {
        ...
        if (attacker.attacking.checkAttackDistance(entity, followingLogic, attacker)) {
            ...
            if(!PathChecker.raycast(attacker, entity, false)){//true for projectiles - what if we rig it?
               // System.out.println("cant attack something through?");
//             if(entity.isNPC())

                return false;
            }
//System.out.println("canmove: "+ entity.getRegionProvider().canMove(attacker.getX(), attacker.getY(), attacker.getHeight(), NPCClipping.getDirection(attacker.getX(), attacker.getY(), entity.getX(), entity.getY()), entity.isNPC()));
...
return !projectile && entity.getRegionProvider().canMove(attacker.getX(), attacker.getY(), attacker.getHeight(), NPCClipping.getDirection(attacker.getX(), attacker.getY(), entity.getX(), entity.getY()), entity.isNPC())
    ...
    public void attackEntity(Entity targetEntity) {
        if (targetEntity == null || !attackEntityCheck(targetEntity, true) || attacker.respawnTimer > 0
        ...
        if (targetEntity.isPlayer()) {
            ...
            if (NPCHandler.transformOnAttack(targetNpc)) {//no this is when YOU attack the target. lol
                return;
            }
        ...
        // Adds the PvP HP overlay when attacking targets top left.
...
//            }
    ...
    private void handleItemChangesOnAttack(Entity targetEntity) {
        ...
        if (attacker.crystalCharge < 1 && attacker.getItems().isWearingItem(25865)) {
            attacker.getItems().unequipItem(25865, 3);
            attacker.sendMessage("Your Bow has ran out of charges.");
            attacker.attacking.reset();
            return;
        }
        if (attacker.getItems().isWearingItem(25865)) {
    ...
    public void fireMageProjectile(Entity entity) {
        ...
        if (attacker.getSpellId() > -1) {
          ...
          //  System.out.println("proj: "+projectile+" from spell: "+attacker.getSpellId());
        ...
        //    int time = 5;//40 + (Math.min(delay, 6) * 10);
            int speed = projectileSpeed(entity);//70 + (8 * distance);//70 + (8 * distance);
...
//            attacker.getPA().createPlayersProjectile(pX, pY, offX, offY, 15, RangeData.getProjectileSpeed(attacker),
...
//                                    Projectile.getLockon(entity), 50);
            ...
            //		<ammunition>
            //			<id>892</id>
            //			<ammunitionType>ARROW</ammunitionType>
            //			<ammunitionMaterial>RUNE</ammunitionMaterial>
            //			<ammunitionPoison>NONE</ammunitionPoison>
            //			<boltTip>NONE</boltTip>
            //			<inHand>false</inHand>
            //			<requiredBows>
            //				<int>855</int>
            //				<int>857</int>
            ...
            //			<graphicsId>24</graphicsId>
            //			<graphicsDelay>15</graphicsDelay>
            //			<graphicsHeight>94</graphicsHeight>
            //			<projectileId>15</projectileId>
            //			<projectileDelay>60</projectileDelay>
            //			<projectileSlowness>5</projectileSlowness>
            //			<startHeight>37</startHeight>
            //			<curve>15</curve>
            //			<endHeight>41</endHeight>
...
//					player.getPlayerAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, RangeData.getProjectileSpeed(player), RangeData.getRangeProjectileGFX(player), 43, 31, -player.oldPlayerIndex - 1, MagicSpells.getStartDelay(player));
    ...
    private boolean determineWeaponStyle() {
        ...
        if (attacker.playerEquipment[Player.playerWeapon] == Items.AK_47 && attacker.playerEquipment[Player.playerArrows] !=  Items.BULLETS_769x39) {
            attacker.sendMessage("You must use 7.62x39 bullets with the AK-47.");
            reset();
            return false;
        }
        ...
        if (!attacker.usingMagic) {
            ...
            attacker.usingCross =
                    attacker.playerEquipment[Player.playerWeapon] == 4734 || attacker.playerEquipment[Player.playerWeapon] == Items.AK_47 || attacker.playerEquipment[Player.playerWeapon] == 9185 || attacker.playerEquipment[Player.playerWeapon] == 11785 || attacker.playerEquipment[Player.playerWeapon] == 26374  || attacker.playerEquipment[Player.playerWeapon] == 21012 || attacker.playerEquipment[Player.playerWeapon] == 21902;
...
//            }
            ...
            if (attacker.getItems().isWearingItem(25865) && attacker.playerEquipment[Player.playerArrows] > -1) {
                attacker.sendMessage("You cannot use any arrows with this bow.");
                attacker.attacking.reset();
                return false;
            }
            ...
            if (!attacker.usingBallista && !attacker.usingCross && !attacker.usingArrows && attacker.usingBow && (((attacker.playerEquipment[Player.playerWeapon] < 4212) || (attacker.playerEquipment[Player.playerWeapon] > 4223)))
                    && !(attacker.playerEquipment[Player.playerWeapon] == 25865) && !(attacker.playerEquipment[Player.playerWeapon] == 25892) && !(attacker.playerEquipment[Player.playerWeapon] == 25890) && !(attacker.playerEquipment[Player.playerWeapon] == 25888) && !(attacker.playerEquipment[Player.playerWeapon] == 29985) && !(attacker.playerEquipment[Player.playerWeapon] == 29988) && !(attacker.playerEquipment[Player.playerWeapon] == 25886) && !(attacker.playerEquipment[Player.playerWeapon] == 25884) && !(attacker.playerEquipment[Player.playerWeapon] == 25867) && !(attacker.playerEquipment[Player.playerWeapon] == 25865) && !(attacker.playerEquipment[Player.playerWeapon] == 27655) && !(attacker.playerEquipment[Player.playerWeapon] == 22550)) {
            ...
            if (!Bow.canUseArrow(attacker) && Configuration.CORRECT_ARROWS && attacker.usingBow && !attacker.getCombatItems().usingCrystalBow() && !(attacker.playerEquipment[Player.playerWeapon] == 27655) && !(attacker.playerEquipment[Player.playerWeapon] == 22550) && attacker.playerEquipment[Player.playerWeapon] != 9185
                    && !(attacker.playerEquipment[Player.playerWeapon] == 25865) && !(attacker.playerEquipment[Player.playerWeapon] == 29985) && !(attacker.playerEquipment[Player.playerWeapon] == 29988) && !(attacker.playerEquipment[Player.playerWeapon] == 25892) && !(attacker.playerEquipment[Player.playerWeapon] == 25890) && !(attacker.playerEquipment[Player.playerWeapon] == 25888) && !(attacker.playerEquipment[Player.playerWeapon] == 25886) && !(attacker.playerEquipment[Player.playerWeapon] == 25884) && !(attacker.playerEquipment[Player.playerWeapon] == 25867)  && !(attacker.playerEquipment[Player.playerWeapon] == 25865)&& attacker.playerEquipment[Player.playerWeapon] != 4734 && attacker.playerEquipment[Player.playerWeapon] != Items.AK_47 && attacker.playerEquipment[Player.playerWeapon] != 26374 && attacker.playerEquipment[Player.playerWeapon] != 11785 && attacker.playerEquipment[Player.playerWeapon] != 21012 && attacker.playerEquipment[Player.playerWeapon] != 12926 && attacker.playerEquipment[Player.playerWeapon] != 19478 && attacker.playerEquipment[Player.playerWeapon] != 19481 && attacker.playerEquipment[Player.playerWeapon] != 21902) {
