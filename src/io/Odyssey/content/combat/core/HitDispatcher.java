...
public abstract class HitDispatcher {
    ...
    private void playerHitEntity(CombatType combatType, Special special, boolean applyingMultiHitAttack) {
        ...
        if (combatType.equals(CombatType.MELEE)) {
            ...
            if (attacker.weaponUsedOnAttack == 25865 && attacker.usingSpecial) {
...
//                if (CombatPrayer.isPrayerOn(attacker, CombatPrayer.DEFLECT_MISSILES)){
...
//            }
             ...
             * Magic attack style
             */
        } else if (combatType.equals(CombatType.MAGE)) {//magic on players AND npcs handled here
            double defenceMultiplier = 1.0;
            double specialAccuracy = 1.0;
            double specialDamageBoost = 1.0;
            double specialPassiveMultiplier = 1.0;
        ...
        if (defender != null && defender.isNPC()) {
            ...
            if (n.getNpcId() == 1543) {
                System.out.println("here? hitdisp");
                if (damage >= n.getHealth().getCurrentHealth()) {
                    if(attacker.getItems().playerHasItem(4162)){
                        //damage = 0;
                    } else {
                        damage = 0;
                        attacker.sendMessage("You need a rock hammer in your inventory to finish off the Gargoyle.");
                    }
        ...
        if (combatType.equals(CombatType.MAGE)) {//what calculates the delay and hit.
        ...
        if(EquipmentSet.AHRIM.isWearingBarrows(attacker)){
            if(Misc.random(100) < 20){
                if (defender.isNPC()) {
                    ...
                    if(definition == null)
                        return;
                    definition.setLevel(NpcCombatSkill.STRENGTH,definition.getLevel(NpcCombatSkill.STRENGTH)-5);//   definition.getLevel(NpcCombatSkill.STRENGTH);
                } else {
                    defender.asPlayer().playerLevel[2] = defender.asPlayer().playerLevel[2]-5;
                }
            }
        }
...
//        dropArrow(itemId);
    ...
    private void dropArrow_mainhand(int arrowId) { // TODO delete arrows from player
        if (Boundary.LONE_SURVIVER.in(attacker))
            return;
        if (attacker.getItems().isWearingItem(10033) || attacker.getItems().isWearingItem(10034)
                || attacker.getItems().isWearingItem(11959)) {
        ...
        int arrow = arrowId;
        int stock = attacker.playerEquipmentN[attacker.playerWeapon];
        int slot = attacker.playerWeapon;

        if (stock > 1) {
            attacker.getItems().setEquipment(arrow, stock - 1, slot,true);
        } else if (stock == 1) {
            attacker.getItems().unequipItem(arrow, slot);
        }
    ...
    public void applyDeflect(Player attacker, Player defender, int damage) {
        if (damage <= 0)
            return;
        ...
        attacker.appendDamage(defender, damage, damage > 0 ? Hitmark.HIT : Hitmark.MISS);
        attacker.addDamageTaken(defender, damage);
...
//            if(CombatPrayer.isPrayerOn(defender.asPlayer(), CombatPrayer.DEFLECT_MAGIC)){
//                applyDeflect(attacker.asPlayer(),defender.asPlayer(),damage);
//            }
//        }
//System.out.println("spellid: "+attacker.getSpellId());
