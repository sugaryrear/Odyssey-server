...
public class MagicCombatFormula implements CombatFormula {
    ...
    public double getAccuracy(Entity attacker, Entity defender, double specialAttackMultiplier,
        ...
        if(attacker.isPlayer()){
            if (Server.getEventHandler().isRunning(attacker.asPlayer(), "sapmage")) {
                double howmuchtodecrease = (attacker.asPlayer().sapDefence * 0.10);
                defence-= (defence) * howmuchtodecrease;
            }
        }
        double accuracy = 0;
        if (attack > defence) {
        ...
        if(attacker.isPlayer() && defender.isPlayer())
            attacker.asPlayer().ismaging = true;
        return accuracy;
    }

    private int getAttackRoll(Entity attacker, Entity defender) {
        double effectiveMagicLevel = attacker.isPlayer() ? getEffectiveAttackLevel(attacker.asPlayer()) : getEffectiveAttackLevel(attacker.asNPC());
