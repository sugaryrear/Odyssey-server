	...
	public static int magiMaxHit(Player c) {
		...
		switch (c.playerEquipment[Player.playerWeapon]) {
			...
			case 24425://eldritch
				...
				break;
			case 23900:
				if (c.getCombatItems().usingCrystalStaffSpecial() && c.wildLevel > 0) {
					damage = getNightmareSpecialMaxHit(c.playerLevel[Skill.MAGIC.getId()], 44); // lowered to 44 from 50
				} else{
					damage = getNightmareSpecialMaxHit(c.playerLevel[Skill.MAGIC.getId()], 120); // lowered to 44 from 50
		...
		boolean hasDarkVersion = (c.petSummonId == 30117 || c.petSummonId == 30120 || c.petSummonId == 29966);
