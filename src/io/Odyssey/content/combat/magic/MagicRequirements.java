...
Double d = 0.85;
					...
					if (Configuration.RUNES_REQUIRED && runesNecessary) {
						if (rune1 > 0) { // deleting runes
							if (!wearingStaff(c, rune1)) {
								...
								if (PetHandler.hassaverunespet(c)){
								//	System.out.println("here: "+ (   Math.round(CombatSpellData.MAGIC_SPELLS[spell][9]*0.85D)   ));
									c.getItems().addItem(rune1,  (int)  Math.round(CombatSpellData.MAGIC_SPELLS[spell][9]*0.85D)   );
								}
							} else {
						...
						if (rune2 > 0) {
							if (!wearingStaff(c, rune2)) {
								...
								if (PetHandler.hassaverunespet(c)){
									c.getItems().addItem(rune2,(int)  Math.round(CombatSpellData.MAGIC_SPELLS[spell][9]*0.85D)  );
								}
							} else {
						...
						if (rune3 > 0) {
							if (!wearingStaff(c, rune3)) {
								...
								if (PetHandler.hassaverunespet(c)){
									c.getItems().addItem(rune3, (int)  Math.round(CombatSpellData.MAGIC_SPELLS[spell][9]*0.85D)  );
								}
							} else {
						...
						if (rune4 > 0) {
							if (!wearingStaff(c, rune4)) {
								...
								if (!PetHandler.hassaverunespet(c)){
									c.getItems().addItem(rune4, (int)  Math.round(CombatSpellData.MAGIC_SPELLS[spell][9]*0.85D));
								}
							} else {
	...
	public static boolean checkMagicReqs_tele(Player c, CityTeleports.TeleportData spelldata, boolean deleteRunes) {
		int spell = spelldata.getSpellId();
		...
		if (spell > -1) {
			...
			//System.out.println("spell: "+spell);
			if (Configuration.MAGIC_LEVEL_REQUIRED) { // check magic level
				if (c.playerLevel[6] < spelldata.getLevel()) {
					c.sendMessage("You need to have a magic level of " + spelldata.getLevel() + " to cast this spell.");
					return false;
				}
			}
			boolean runesNecessary = true;
