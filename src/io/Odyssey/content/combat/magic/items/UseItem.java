...
public class UseItem {
	...
	public static void unNoteItems(Player c, int itemId, int amount) {
		...
		int counterpartId = ItemStats.ItemList[itemId].getCounterpartId();
//ItemStats.ItemList[itemId].getCounterpartId()+1
		/**
...
//		return;
	...
	public static void ItemonObject(Player c, int objectID, int objectX, int objectY, int itemId) {
		...
		if (c.getQuesting().handleItemonObject(itemId, objectID)) {
			return;
		}
		if (def != null) {
		...
		switch (objectID) {
			case 3827://kalphite lair
				if (!c.getItems().playerHasItem(ROPE, 1)) {
			c.sendMessage("You need a @blu@Rope@bla@ to go down there.");
					return;
				}
				c.getItems().deleteItem(ROPE,1);
				c.getPA().checkObjectSpawn(3828, objectX, objectY, c.heightLevel,0,10);
				break;
			case 37341:
		...
		case 16469:
		case 2030:
			case 24009://Allows for ores to be used on the furnace instead of going though the interface.
				switch(itemId){
					case Items.RING_MOULD:
						JewelryMaking.mouldInterface(c,Items.RING_MOULD);
						break;
					case Items.AMULET_MOULD:
						JewelryMaking.mouldInterface(c,Items.AMULET_MOULD);
						break;
					case Items.NECKLACE_MOULD:
						JewelryMaking.mouldInterface(c,Items.NECKLACE_MOULD);
						break;
					case Items.BRACELET_MOULD:
...
//					}
		...
		case 26181:
			case 9682:
			if(objectID == 5249){
					if(!Server.getGlobalObjects().anyExists(objectX, objectY, c.heightLevel)) {
						c.sendMessage("The fire has been extinguished.");
						return;
					}
			}
			c.facePosition(objectX, objectY);
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (OrnamentedItem.ornament(c, itemUsed, useWith))
			return;
		if (itemUsed == 4155 && useWith == 8901 || itemUsed == 8901 && useWith == 4155) {
...
//			}
//		}
		if (itemUsed == 23877 && useWith == 25865) {
		...
		if (itemUsed == 21820 && useWith == 21817) { //bracelet of ether charged
			int etheramount = c.getItems().getItemAmount(21820);
			int maxEtherToAdd = 16000 - c.braceletEtherCount;
			if ((c.braceletEtherCount + etheramount) >= 16000) {
				etheramount = maxEtherToAdd;
			...
			c.braceletIncrease(etheramount);
			c.getItems().deleteItem(21820, etheramount);
			c.getItems().replaceItem(c,21817, 21816);
			c.sendMessage("You now have " + c.braceletEtherCount + " ether in your bracelet");
			return;
		}
		if (( itemUsed == 12929 || useWith == 12929)) {//uncharged helms
		...
		if (( itemUsed == 12931 || useWith == 12931)) {//charged helms
			if (useWith == 13200 || itemUsed == 13200 || useWith == 13201 || itemUsed == 13201) {
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().deleteItem2(itemUsed, 1);
				int mutagen = useWith == 13200 || itemUsed == 13200 ? 13197 : 13199;
				c.getItems().addItem(mutagen, 1);
			}
		}
		...
		if (itemUsed == 554 || itemUsed == 560 || itemUsed == 562) {
			if (useWith == 11907)
				c.getDH().sendOption3("10", "100", "1000");
			c.dialogueAction = 55;
		}
...
//		}
	...
	public static void handleItemOnNPC(Player player, NPC npc, int itemId, int slot) {
		...
		if(npcId == 7690){
			if (itemId == 6570) {
				player.getDH().sendOption2("@red@Really sacrifice your Fire cape?", "No, I'd like to keep it.", "Yes, I'd like to sacrifice it.");
				player.dialogueAction = 1260;
			}
		}
		if (npcId == 954) {
