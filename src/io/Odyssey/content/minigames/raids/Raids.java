...
import io.Odyssey.content.item.lootable.impl.RaidsChestRare;
...
//        }
        if (chance >= 0 && chance < rareChance) {
          ...
          //  player.sendMessage("@red@You have just received a @bla@Mystery box.");
    ...
    public void handleMobDeath(Player killer, int npcType) {
        ...
        switch (npcType) {
            case OLM:
                ...
                // *
                olmDead = true;
			//Server.getGlobalObjects().add(new GlobalObject(-1, 3233, 5751, currentHeight, 3, 10));
		//	Server.getGlobalObjects().add(new GlobalObject(-1, 3232, 5749, currentHeight, 3, 10));
////the crystal
                //why is it not spawning the chest?
         Server.getGlobalObjects().add(new GlobalObject(30028, 3234, 5753, currentHeight, 0, 10));
//
//
//
//                GlobalObject theotherobj_ = new GlobalObject(30027, 3233, 5751, currentHeight, 0, 10);
//                Server.getGlobalObjects().add_door(theotherobj_);
//                GlobalObject theotherobj2_ = new GlobalObject(30018, 3232, 5749, currentHeight, 0, 10);
//                Server.getGlobalObjects().add_door(theotherobj2_);


             //  Server.getGlobalObjects().add(new GlobalObject(30018, 3232, 5749, currentHeight, 0, 10));
             //   Server.getGlobalObjects().add(new GlobalObject(30027, 3233, 5751, currentHeight, 0, 10));

                Server.getGlobalObjects().remove(30018, 3232, 5749, currentHeight); // Remove
                Server.getGlobalObjects().remove(30027, 3233, 5751, currentHeight); // Remove


                getPlayers().stream().forEach(player -> {
//player.getPA().movePlayer(3234, 5753);
                    player.getPA().sendPlayerObjectAnimation(3220, 5738, 7348, 10, 3);
                    player.sendMessage("@red@Congratulations you have completed Chambers of Xeric!");
...
//         theotherobj = killer.getRegionProvider().get(killer.absX, killer.absY).getWorldObject(29884, 3220, 5743,killer.getHeight());
//         theotherobj_ = new GlobalObject(theotherobj.get().id, theotherobj.get().x, theotherobj.get().y, theotherobj.get().height, theotherobj.get().face, theotherobj.get().type);
//
//
//                Server.getGlobalObjects().add(theotherobj_);
//                Server.getGlobalObjects().remove(theotherobj_);
//                Server.getGlobalObjects().updateObject(theotherobj_, -1);
...
//                theotherobj = killer.getRegionProvider().get(killer.absX, killer.absY).getWorldObject(30018, 3220, 5743,killer.getHeight());
//                theotherobj_ = new GlobalObject(theotherobj.get().id, theotherobj.get().x, theotherobj.get().y, theotherobj.get().height, theotherobj.get().face, theotherobj.get().type);
//
//
//                Server.getGlobalObjects().add(theotherobj_);
...
//                    theotherobj = killer.getRegionProvider().get(killer.absX, killer.absY).getWorldObject(30018, 3232, 5749,killer.getHeight());
//                    theotherobj_ = new GlobalObject(theotherobj.get().id, theotherobj.get().x, theotherobj.get().y, theotherobj.get().height, theotherobj.get().face, theotherobj.get().type);
//
//
//                    Server.getGlobalObjects().add(theotherobj_);
