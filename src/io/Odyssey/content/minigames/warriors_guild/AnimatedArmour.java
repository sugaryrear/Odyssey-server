...
public static int clickanimator(final Player player){


	Armour s = null;
	for (int i = 0; i < player.playerItems.length; i++) {
		for (Armour a : Armour.values())
			if (a.getHelmId()+1 == player.playerItems[i] || a.getPlatebodyId()+1 ==player.playerItems[i] || a.getPlatelegsId()+1 == player.playerItems[i])
				s = a;
	}
	if(player.getItems().playerHasItem(s.helm) && player.getItems().playerHasItem(s.platebody) && player.getItems().playerHasItem(s.platelegs)){
		return s.helm;
	}
	return -1;
}
	public static void itemOnAnimator(final Player player, int itemId) {
		if(itemId == -1){
			player.sendMessage("You don't have any suitable items to use on the animator.");
			return;
		}
