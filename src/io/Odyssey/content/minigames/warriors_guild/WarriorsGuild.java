...
public class WarriorsGuild {
	...
	public static final int[][] DEFENDER_DATA = { { 8844, 10 }, { 8845, 15 }, { 8846, 20 }, { 8847, 22 }, { 8848, 25 }, { 8849, 30 }, { 8850, 35 } };

	public WarriorsGuild(Player player) {
		this.player = player;

	}
	public  void changedanimatordoor(int objectType, int obX, int obY) {

		if(objectType == 24306){
			player.getPA().object(objectType, obX , obY, 0, 0);
			player.getPA().object(objectType+3, obX+1 , obY, 2, 0);
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					player.getPA().object(objectType+3, obX+1 , obY, 3, 0);
			...
			player.getPA().object(objectType, obX , obY, 2, 0);
			player.getPA().object(objectType-3, obX-1 , obY, 0, 0);
	...
	public void walkthrudoor(boolean entering) {
		if (entering) {
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				@Override
				public void execute(CycleEventContainer container) {
					if (player.getAgilityHandler().hotSpot(player, 2846, 3540)) {//change x y to left of door
						player.getPA().object(24309, 2487,3541, 0, 0);//change x y
						player.getPA().object(24306, 2487 ,3540, 2, 0);
						//can update for others if u want lol
						player.getAgilityHandler().move(player, (player.absX - player.objectX) + 1, (player.objectY - player.absY), 819, -1);
					...
					player.getAgilityHandler().move(player, (player.absX - player.objectX), (player.objectY - player.absY), 819, -1);
			...
			CycleEventHandler.getSingleton().stopEvents(this);
//		player.getPA().object(24309, 2487,3541, 0, 0);
//		player.getPA().object(24306, 2487 ,3540, 2, 0);
			player.getAgilityHandler().move(player, (player.objectX -player.absX), (player.objectY - player.absY), 819, -1);
		...
		public void closedoorevent() {
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					player.getPA().object(10043, 2911, 9968, 3, 0);
					container.stop();
				}
