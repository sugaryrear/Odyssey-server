...
public class LootingBag {
    ...
    public void depositAllLootBag() {
        ...
        if (!player.getPosition().inBank()) {
            player.sendMessage("You must be near a bank to do this.");
            return;
        }
        ...
        for (Iterator<LootingBagItem> iterator = getLootingBagContainer().items.iterator(); iterator.hasNext(); ) {
            ...
            //ItemDef.forId(id).isNoted()
            player.getItems().addItemToBankOrDrop(ItemDef.forId(item.getId()).isNoted()  ? item.getId()-1 : item.getId(), item.getAmount());
            iterator.remove();
        }
        updateItemContainers();
    }
