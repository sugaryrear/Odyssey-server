...
public class Task extends Command {
    ...
    public void execute(Player player, String commandName, String input) {
        ...
        if (player.getSlayer().getMaster() == 2981) {//honourguard5 nex slayer
            player.sendMessage("I currently have " + player.getSlayer().getTaskAmount() + " " + player.getSlayer().getTask().get().getPrimaryName() + "'s to kill in "  + player.getHonourguard5SlayerLocation() + ".");
        } else {
            player.sendMessage("I currently have " + player.getSlayer().getTaskAmount() + " " + player.getSlayer().getTask().get().getPrimaryName() + "'s to kill.");
        }
