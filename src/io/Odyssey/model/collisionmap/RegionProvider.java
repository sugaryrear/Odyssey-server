...
public class RegionProvider {
	...
	/**
	 * An array of fully blocked NPCs that block movement and projectiles YOU WONT BE ABLE TO STAND OR MOVE PAST THIS NPCS TILE!
	 */
	public static final int[] FULL_BLOCKED_NPCS = {//nice
			Npcs.GORILLA, Npcs.BEARDED_GORILLA, Npcs.ELDER_GUARD, Npcs.ELDER_GUARD_2,
			7710// ROCKY SUPPORT

			//,2256,2250,2253//waterbirth isle door supports
	};
...
//		if (Boundary.isIn(new Position(x, y, height), Boundary.CATACOMBS)//why no clip in catacombs?
...
//System.out.println("x: "+x+" height: "+height);
	...
	public void removeClip(int flag, int x, int y, int height) {
		// Blocks clipping from being added again to already clipped NPC tiles
//		if (!isOccupiedByNpc(x, y, height) && !isOccupiedByFullBlockNpc(x, y, height))
//			return;
//
...
//			return;
	...
	public boolean projectileBlockedSouth(int x, int y, int z) {//what checks below you!
		return isOccupiedByFullBlockNpc(x, y - 1, z)
