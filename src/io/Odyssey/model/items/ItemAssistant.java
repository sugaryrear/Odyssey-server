...
public class ItemAssistant {
	public void addRandomBox(int itemId){
		player.getItems().addItemUnderAnyCircumstance(787,1);
		player.sendMessage("You notice a @blu@Resource box@bla@ in your inventory...");
	}
	...
	public void addContainerUpdate(ContainerUpdate containerUpdate) {//lmao
	...
	public int getBankItemsamt(int id) {
		int amt = 0;
		List<GameItem> bankItems = new ArrayList<>();
		for (BankTab tab : player.getBank().getBankTab())
			for (BankItem bankItem : tab.getItems())
				if(bankItem.getId() == id)
				amt++;
		return amt;
	}
	...
	public void addItemToBankOrDrop(int itemId, int amount) {
		...
		if (ItemDef.forId(itemId).isNoted()) {
			...
			System.out.println("item id added: "+item.getId());
		}
	...
	public void calculateBonuses() {
		...
		for (int playerEquipment : player.playerEquipment) {
			if (playerEquipment > -1) {
				...
				Bonuses entirebonusesarray = Bonuses.getbonuses(playerEquipment);
...
//stab, slash, crush, range, mage,stabdef,slashdef,crushdef,rangedef,magedef,str,rangestr,magestr,pray
				...
				if(entirebonusesarray  != null){
					player.playerBonus[Bonus.ATTACK_STAB.ordinal()] += entirebonusesarray.bonuses()[0];
					player.playerBonus[Bonus.ATTACK_SLASH.ordinal()] += entirebonusesarray.bonuses()[1];
					player.playerBonus[Bonus.ATTACK_CRUSH.ordinal()] += entirebonusesarray.bonuses()[2];
					player.playerBonus[Bonus.ATTACK_RANGED.ordinal()] += entirebonusesarray.bonuses()[3];
					player.playerBonus[Bonus.ATTACK_MAGIC.ordinal()] += entirebonusesarray.bonuses()[4];
					player.playerBonus[Bonus.DEFENCE_STAB.ordinal()] += entirebonusesarray.bonuses()[5];
					player.playerBonus[Bonus.DEFENCE_SLASH.ordinal()] += entirebonusesarray.bonuses()[6];
					player.playerBonus[Bonus.DEFENCE_CRUSH.ordinal()] += entirebonusesarray.bonuses()[7];
					player.playerBonus[Bonus.DEFENCE_RANGED.ordinal()] += entirebonusesarray.bonuses()[8];

					player.playerBonus[Bonus.DEFENCE_MAGIC.ordinal()] += entirebonusesarray.bonuses()[9];
					player.playerBonus[Bonus.STRENGTH.ordinal()] += entirebonusesarray.bonuses()[10];
					player.playerBonus[Bonus.RANGED_STRENGTH.ordinal()] += entirebonusesarray.bonuses()[11];
					player.playerBonus[Bonus.MAGIC_DMG.ordinal()] += entirebonusesarray.bonuses()[12];
					player.playerBonus[Bonus.PRAYER.ordinal()] += entirebonusesarray.bonuses()[13];
				}
				//}
	...
	private void updateEquipmentBonusInterface() {
		...
		player.getPA().sendFrame126(DropManager.getModifier1(player)+"%", 70010);
	...
	public void sendWeapon(int weapon) {
		...
		String WeaponName2 = ItemDef.forId(weapon).getName().replaceAll("Bronze", "");
		...
		WeaponName2 = WeaponName2.replaceAll("Abyssal", "");
		WeaponName2 = WeaponName2.replaceAll("Armadyl", "");
		WeaponName2 = WeaponName2.replaceAll("Zamorak", "");
		WeaponName2 = WeaponName2.replaceAll("Saradomin", "");
		WeaponName2 = WeaponName2.replaceAll("Ancient", "");
		WeaponName2 = WeaponName2.replaceAll("Bandos", "");
		WeaponName2 = WeaponName2.trim();


	//String wepname = ItemDef.forId(weapon).getName().replaceAll

		player.getPA().sendString("Category: "+WeaponName2, 19995);
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		switch (itemId) {//just add items that should be 2h like training bow here.
			case 9705:
		case 12926:
		case 6724:
		case 11838:
...
//			break;
	...
	public boolean equipItem(int wearID, int slot) {
		...
		if (player.playerItems[slot] == (wearID + 1)) {
			...
			Map<Integer, Integer> reqs = EquipmentInfo.getInstance().requirementsFor(wearID);
			if (reqs != null && reqs.size() > 0) {
				reqs.forEach((key, value) -> {
					if (!needsreq[0] && player.playerLevel[key]  < value) {
						player.sendMessage("You need %s %s level of %d to equip this.",SKILL_INDEFINITES[key], SKILL_NAMES[key], value);
						//player.sendMessage("You need a skill level (skill name based on level goes here) of blah blah "+value+" ");
						needsreq[0] = true;
					}
				});
			}
			...
			if (needsreq[0]) {
				player.sendMessage("<col=FF0000>You don't have the level requirements to wear this item.");
				return false;
			}
			if(wearID == 25865 && player.crystalCharge < 1){
				player.sendMessage("This bow can be charged by crystal shards in order to be able to worn.");
				return false;
...
//System.out.println("id: "+itemId);
			...
			//System.out.println("here? id: "+item.getId());
		}
	...
	public boolean addToBankdepositbox(int itemId, int amount, boolean updateView, boolean overrideValidation) {
	...
	public void deleteFromBank(BankTab tab, int itemId, int itemAmount, boolean updateView) {
		BankItem item = new BankItem(itemId + 1, itemAmount);
		boolean noted = false;
...
//		}
	...
	public void removeFromBank(BankTab tab, int itemId, int itemAmount, boolean updateView) {
		...
		//	System.out.println("noted? :"+ItemDef.forId(itemId).isNoted()+" and counterpartA: "+Server.itemHandler.getCounterpart(itemId));
		if( ItemStats.ItemList[itemId] == null){
System.out.println("itemlist is null: "+itemId);

			return;
		}
			if (!ItemDef.forId(itemId).isNoted() && ItemStats.ItemList[itemId].getCounterpartId() > 0) {
				noted = true;
			} else {
		...
		if (item.getAmount() > 0) { // Can't release placeholder now with the addItem if statement check
			if (!noted) {
				...
				if (!addItem(ItemStats.ItemList[item.getId()].getCounterpartId()+1, item.getAmount()))
//				if (!addItem(Server.itemHandler.getCounterpart(item.getId() - 1), item.getAmount()))
	...
	public List<Integer> deleteItemAndReturnAmount_equipment(int id, int amount) {
		if (id <= 0) {
			return null;
		}
		int count = 0;
		...
		for (int j = 0; j < player.playerEquipment.length; j++) {
			...
			if (player.playerEquipment[j] == (id + 1)) {
				if (player.playerEquipmentN[j] > amount) {
					player.playerEquipmentN[j] -= amount;
					count += amount;
					amountsToReturn.add(amount);
				} else {
					count += player.playerEquipmentN[j];
					amountsToReturn.add(player.playerEquipmentN[j]);
					player.playerEquipmentN[j] = 0;
					player.playerEquipment[j] = 0;
				}
			}
		}
		this.addContainerUpdate(ContainerUpdate.EQUIPMENT);
	...
	public void deleteArrow() {
		...
		if (player.getItems().isWearingItem(10499, player.playerCape) || player.getItems().isWearingItem(22109, player.playerCape) ||SkillcapePerks.RANGING.isWearing(player)
				|| player.getItems().isWearingItem(33037, player.playerCape)|| SkillcapePerks.isWearingMaxCape(player) && !player.getItems().isWearingItem(4734)  && !player.getItems().isWearingItem(Items.AK_47)&&
				!player.getItems().isWearingItem(10033) && !player.getItems().isWearingItem(10034) && !player.getItems().isWearingItem(11959)) {
			if (RandomUtils.nextInt(0, 100) < 72) {
				return;
			}
		}
	...
	public void deleteEquipment() {
		...
		if ( (Arrays.stream(WeaponDataConstants.THROWN).anyMatch(id -> id == thrownweapons) || player.usingOtherRangeWeapons) && !player.getItems().isWearingItem(Items.TOXIC_BLOWPIPE)) {
			if (player.getItems().isWearingItem(10499, player.playerCape) || player.getItems().isWearingItem(22109, player.playerCape)
					...
					|| SkillcapePerks.isWearingMaxCape(player) && !player.getItems().isWearingItem(4734)  && !player.getItems().isWearingItem(Items.AK_47)) {
				if (RandomUtils.nextInt(0, 100) < 72) {
					return;
				}
			}
	...
	public void createGroundItem(GroundItem groundItem) {
		//System.out.println("item: "+groundItem.ownerName+" "+groundItem.getId()+" "+groundItem.getHeight());
		if (player.getOutStream() != null) {
	...
	public void handleAmethyst() {
		if (!player.boltTips&&!player.arrowTips&&!player.javelinHeads) {
			player.start(new AmethystChiselDialogue(player));
			return;
		}
