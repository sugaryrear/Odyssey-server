...
public class ItemHandler {
	...
	public boolean itemExists_globaldrop(int itemId, int itemX, int itemY, int height) {
...
//
		player.getLocalGroundItems().stream().filter(it -> it.inDistance(player))
				.forEach(it -> player.getItems().removeGroundItem(it, false));
		player.getLocalGroundItems().clear();
//maybe something around here will fix the items not displaying on height 2 ?
	...
	public void createUnownedGroundItem_globaldrop(GameItem gameItem, Position position) {
		createUnownedGroundItem_globaldrop(gameItem.getId(), position.getX(), position.getY(), position.getHeight(), gameItem.getAmount());
	}

	public void createUnownedGroundItem_globaldrop(int id, int x, int y, int height, int amount) {
		if (id > 0 && amount > 0) {
			if (id >= 2412 && id <= 2414) {
				return;
			}
			if (!ItemDef.forId(id).isStackable() && amount > 0) {
				...
				for (int j = 0; j < amount; j++) {
					GroundItem item = new GroundItem(id, x, y, height, 1, 2, "");
					items.add(item);
					//logger.debug("Added ground item: " + item);
				}
			} else {
				GroundItem item = new GroundItem(id, x, y, height, amount,2, "");
				items.add(item);
				//logger.debug("Added ground item: " + item);
			}
		}
	...
	public void createGroundItem(Player player, int itemId, int itemX, int itemY, int height, int itemAmount, int playerId, boolean globalise, int hideTicks) {
		...
		if (itemId > 0 && itemAmount > 0) {
			...
			for (int item = 0; item < Configuration.VANISH_ITEMS.length; item++) {
				if(itemId == Configuration.VANISH_ITEMS[item]) {
					player.sendMessage("The@blu@ " + ItemAssistant.getItemName(itemId) + "@bla@ vanishes as it touches the ground.");
					return;
				}
			}
			/**
			...
			//	System.out.println("login name of dead: "+owner.getLoginName());
				player.getItems().createGroundItem(item);
				item.setGlobalise(globalise);
				item.setInstance(player.getInstance());
				items.add(item);
