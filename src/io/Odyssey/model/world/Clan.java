...
public class Clan {
	...
	public void updateDisplayName(Player player) {
		if (activeMembers.removeIf(it -> it.getDisplayName().equalsIgnoreCase(player.getDisplayName()))) {
			activeMembers.add(new ClanMember(player.getDisplayName(), player.getDisplayName()));
			sort();
			setDirty(true);
		}
	}
	...
	public void addMember(Player player) {
		if (isBanned(player.getDisplayName())) {
			player.sendMessage("<col=FF0000>You are currently banned from this clan chat.</col>");
			return;
		}
		if (whoCanJoin > Rank.ANYONE && !isFounder(player.getDisplayName())) {
			if (getRank(player.getDisplayName()) < whoCanJoin) {
	...
	private boolean remove(Player player) {
		if (activeMembers.removeIf(it -> it.getDisplayName().equals(player.getDisplayName()))) {
			sort();
			setDirty(true);
			return true;
		}
	...
	public void updateInterface(Player player) {
		...
		player.getPA().sendString(33815, "CC (" + activeMembers.size() + "/200)");

		for (int index = 0; index < 100; index++) {
			if (index < activeMembers.size()) {
			//	System.out.println("rank: "+getRank(activeMembers.get(index).getLoginName()) );
				player.getPA().sendFrame126("<clan=" + getRank(activeMembers.get(index).getDisplayName()) + ">" + activeMembers.get(index).getDisplayName(), 18144 + index);
			} else {
	...
	public void resetInterfaceOnLeave(Player player) {
		...
		player.getPA().sendString(33815, "CC (0/200)");
		for (int index = 0; index < 100; index++) {
	...
	public void sendChat(Player paramClient, String paramString) {
		if (getRank(paramClient.getDisplayName()) < this.whoCanTalk) {
				...
				if (c != null && activeMembers.stream().anyMatch(it -> it.is(c))) {
					if (!c.getFriendsList().getRepository().isIgnore(paramClient)) {
						c.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] " + paramClient.getDisplayNameFormatted() + ": @dre@"
								+ paramString.substring(1));//yep what sends the entire string to client.
					}
				}
			}
		}
...
//		}
	...
	public void delete() {
		for (ClanMember name : activeMembers) {
			remove(name.getDisplayName());

			Player player = PlayerHandler.getPlayerByLoginName(name.getDisplayName());
			if (player != null) {
				resetInterfaceOnLeave(player);
	...
	public Clan(Player player) {
		...
		setFounder(player.getDisplayName().toLowerCase());
	}
