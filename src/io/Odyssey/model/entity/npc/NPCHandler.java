...
import static io.Odyssey.model.Npcs.GIANT_MOLE;
...
public class NPCHandler {
    ...
    /**
     * Attack delay
     * @return the delay were setting - how often it attacks?
     */
    public int getNpcDelay(NPC npc) {
        NpcCombatDefinition definition = npc.getCombatDefinition();
        if (definition != null)
            return definition.getAttackSpeed() <= 4 ? 5 :  definition.getAttackSpeed();//MUST NOT BE LESS THAN 5
        else
            switch (npc.getNpcId()) {
                case 2043:
                    return 9;
...
//this is what spawns ALL the npcs to the world on load
    ...
    void respawn(int i, Player owner) {
        ...
        try {
            ...
            newNpc.children = children;//children of spawned npcs
            if (instance != null) {
                instance.add(newNpc);
            }
    ...
    public static Optional<RFDInstance> getRFDInstance(NPC npc) {
        if (npc.getInstance() != null && npc.getInstance() instanceof RFDInstance) return Optional.ofNullable((RFDInstance) npc.getInstance());
        return Optional.empty();
    }
    public static Optional<Hunllef> getHunlleffInstance(NPC npc) {
        if (npc.getInstance() != null && npc.getInstance() instanceof Hunllef) return Optional.ofNullable((Hunllef) npc.getInstance());
        return Optional.empty();
    }
    /**
            ...
            if (PlayerHandler.players[j] != null) {
                ...
                if (PlayerHandler.players[j].goodDistance(c.absX, c.absY, npc.absX, npc.absY, multiAttackDistance(npc))) {
                    ...
                    if (npc.getAttackType() == CombatType.SPECIAL) {
                        ...
                        c.gfx100(npc.endGfx);//dragonfire gfx
                        c.appendDamage(npc, damage, hitmark);
                    ...
                    if (npc.endGfx > 0) {
                        c.gfx0(npc.endGfx);//multi attack damage graphics end
                    }
        ...
        if (npc != null) {
            if (player != null) {
                if (player.getFightCave() != null) {
                    if(npc.getNpcId() != Npcs.YT_HURKOT)///yt-hurkot doesn't count towards KC
    ...
    public void appendtaskkc(NPC npc) {
        ...
        Tasks specifictask = player.getTaskMasterManager().gettaskbynpcid(npc.getNpcId());//beautiful
        if(specifictask != null)
            player.getTaskMasterManager().increase(specifictask);
        //  }
    }
    ...
    public static List<NPC> getNpcs(int npcType, int height) {
        List<NPC> freshlist = new ArrayList<>();
        for (NPC npc : npcs) {
    ...
    public void followPlayer(NPC npc, int playerId) {
        ...
        if (npc.getNpcId() >= 2250 && npc.getNpcId() <=2256) {
            return;
        }
        ...
        if (npc.getNpcId() == 5961) {//spinolyp in dagannoth dungeon
    ...
    public int distanceRequired(NPC npc) {
        ...
        case 8781: 
            return 6;
        case 239: 
            return npc.getAttackType() == CombatType.DRAGON_FIRE ? 18 : 1;
        case 8030: 
        case 8031: 
        ...
        default: 
            return  npc.getAttackType() != null && (npc.getAttackType() == CombatType.RANGE  || npc.getAttackType()== CombatType.MAGE || npc.getAttackType()== CombatType.SPECIAL) ? 7 : 1;
    ...
    public int followDistance(NPC npc) {
        if (Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS) || Boundary.isIn(npc, Boundary.CORPOREAL_BEAST_LAIR) || Boundary.isIn(npc, Boundary.CERBERUS_BOSSROOMS) || Boundary.isIn(npc, Boundary.NexAngelOfDeath))
            if(Boundary.isIn(npc, Boundary.NEX))
                return 10;
        else
            return 40;
        if (Boundary.isIn(npc, Boundary.XERIC) || Boundary.isIn(npc, Boundary.HUNLLEF_BOSS_ROOM)) {
            return 128;
        }
    ...
    public static boolean ignoresProjectile(NPC npc) {
       ...
       case Brother.AHRIM:
        ...
        case 8030:
            ...
            case 3127://obv jad ignores projectiles  so you can do it .
            return true;
        }
        return false;
    }
    ...
    public void attackPlayer(Player c, NPC npc) {
        ...
        if (c.isInvisible() || npc.isInvisible()) {
            return;
        }
        ...
        if (npc != null) {
            ...
            case 8062:
                ...
                npc.setDead(true);
                break;
                //here is where u do spinolyp transform lmao.
                //when  get near hahahahaha
            ...
            if (definition != null)
                npc.setAttackType(CombatConfigs.getCombatTypeForNpc(definition));
            else
          ...
          //  System.out.println("def after: "+npc.getAttackType().name());
            boolean hasDistance = npc.getDistance(c.getX(), c.getY()) <= ((double) distanceRequired(npc)) + (npc.getSize() > 1 ? 0.5 : 0.0);



            if (hasDistance) {
                if (npc.getAttackType() == CombatType.MELEE) {// this fixes attacking through walls - buggy af.
                    if(!ignoresProjectile(npc))//if its true the npc can hit you from anywhere through anything (like zulrah or barrows brothers range/mage guys)
                    if(!PathChecker.raycast(c, npc, false)){//true for projectiles - what if we rig it?
                   //   System.out.println("check here? an npc attacking with melee, and doesnt ignore fences (this is normal), and it cant find clear path between it and u - which it should.");
                        return;
                    }
                } else {
                    if(!ignoresProjectile(npc))
                        if (!PathChecker.raycast(npc, c, true)
                            ...
                            //here is npcs that really do use projectiles.
                            return;
                }


                if (c.respawnTimer <= 0) {//lol so youre just saying that if the player is ALIVE then attack them..lol ok.

                    if (npc.getNpcId() == Npcs.GIANT_MOLE && GiantMole.moleburying(npc, c)) {
                        return;
                    }
                    startAnimationHighPriority(getAttackEmote(npc), npc);//make sure this is AFTER the above (checks the combattype for the animation)

                    npc.attackTimer = getNpcDelay(npc);//how OFTEN the npc attacks//
                    npc.hitDelayTimer =  npc.getAttackType() == CombatType.MELEE ? 2 : getDelay(npc,CombatConfigs.getCombatTypeForNpc(definition),c);//the time until the damage is applied
...
//maybe dont close the interface of barrows puzzle? sure! eh..not here actually lol.
                    if (c.getOpenInterface() != 39960) {//oh so if the interface ISN'T 39960 it will close all windows when the npc attacks you
    ...
    public static int getDelay(NPC attacker,CombatType combattype, Player entity)
    {
      ...
      //  int offY = (pX - nX) * -1;
       ...
       // System.out.println("before hit: "+hitDelay);
if(distance <= 2 && combattype != CombatType.MELEE){
    hitDelay=3;
}
     //   System.out.println("after hit: "+hitDelay);
        int time = 5;//40 + (Math.min(delay, 6) * 10);

        return hitDelay;
    }
//    public int projectileSpeed(NPC attacker, Entity target) {
//        int clientSpeed;
//        if (attacker.goodDistance(target.getX(), target.getY(), attacker.getX(), attacker.getY(), 1)) {
//            clientSpeed = 56;
//        } else if (attacker.goodDistance(target.getX(), target.getY(), attacker.getX(), attacker.getY(), 5)) {
//            clientSpeed = 61;
//        } else if (attacker.goodDistance(target.getX(), target.getY(), attacker.getX(), attacker.getY(),  8)) {
//            clientSpeed = 71;
//        } else {
//            clientSpeed = 81;
//        }
//        return clientSpeed;
//    }
    public int getProjectileHitDelay(NPC target, Player c) {

        int gfxDelay;
        Position pos = new Position(target.getX(),target.getY());
        if (c.getPosition().withinDistance(pos, 1)) {
            gfxDelay = 80;
        } else if (c.getPosition().withinDistance(pos, 5)) {
            gfxDelay = 100;
        } else if (c.getPosition().withinDistance(pos, 8)) {
            gfxDelay = 120;
        } else {
            gfxDelay = 140;
        }
...
//Add npcs who will straight up ignore if you have a prayer on and still hit you through it
...
//edit npc max hit here
    public int getMaxHit(Player c, NPC npc) {
        ...
        if (Arrays.stream(GellatinothMother.DAGANNOTH_MOTHER_TRANSFORMS).anyMatch(dagId -> dagId == npc.getNpcId())) {
            if (npc.getAttackType() == CombatType.MELEE) {
                return 9;
            } else {
                return 12;
        ...
        case 239: 
            return npc.getAttackType() == CombatType.DRAGON_FIRE ? 65 : 22;
        case 465: 
