...
public class PetHandler {
    ...
    private static final ImmutableSet<Integer> MAGE_PETS = ImmutableSet.of(
        ...
        private static final ImmutableSet<Integer> HEALING_PET = ImmutableSet.of(
            Pets.ZILLY.itemId
    );
    private static final ImmutableSet<Integer> DROP_PET = ImmutableSet.of(
            Pets.DEVIL.itemId
    );
    private static final ImmutableSet<Integer> RANGE_PET = ImmutableSet.of(
            Pets.KITTEN.itemId
    );
    private static final ImmutableSet<Integer> RUNES_PET = ImmutableSet.of(
            Pets.MONKEY.itemId
    );
    private static final ImmutableSet<Integer> ROL_PET = ImmutableSet.of(
            Pets.DARK_CORE.itemId
    );
    ...
    /**
     * add pets that u want to heal here
     * @param player
     * @return
     */
    public static boolean hasHealingPet(Player player) {
        return HEALING_PET.contains(player.petSummonId);
    }
    public static boolean hasdROPPet(Player player) {
        return DROP_PET.contains(player.petSummonId);
    }
    public static boolean hasrangedpet(Player player) {
        return RANGE_PET.contains(player.petSummonId);
    }
    public static boolean hassaverunespet(Player player) {
        return RUNES_PET.contains(player.petSummonId);
    }
    public static boolean hasringoflifepet(Player player) {
        return ROL_PET.contains(player.petSummonId);
    }
...
//ADD NPCS THAT WILL HAVE PETS DISPLAYED ON THE DROP TABLE HER
    public enum Pets {
        GRAARDOR(12650, 6632, "General Graardor", 500, "second"),
        KREE(12649, 6643, "Kree'Arra", 500, "second"),
        ZILLY(12651, 6633, "Commander Zilyana", 500,"second", 5),
        ...
        MOLE(12646, 6651, "Giant Mole", 3000, "second"),
...
//        VOTE_GENIE_PET2(21262, 326, "Vote Genie Pet", 1000, "second"),
        ...
        PAK_YAK_1(31014, 15565, "Pack Yak", -1, "second"),
        PAK_YAK_2(31014, 15572, "Pack Yak", -1, "second"),
        PAK_YAK_3(31014, 15573, "Pack Yak", -1, "second"),
...
//29964
        ...
        Pets(int itemId, int npcId, String parent, int droprate, String pickupOption,int hp) {
            this(true, itemId, npcId, parent, droprate, pickupOption);
            this.hpheal = hp;
        }
...
//                return Pets.VOTE_GENIE_PET;
    ...
    public static void spawn(Player player, Pets pet, boolean ignore, boolean ignoreAll) {
        ...
        if (pet.itemId == 12840 && !ignore) {
            ...
            player.petSummonId = pet.itemId;
            PlayerSave.saveGame(player);
            int thenpctospawn = pet.npcId;
            if(pet.getSummonId() == Pets.PAK_YAK_1.getSummonId())//if youre tring to summon a pak yak
                //lets check what level were at
                if(player.yakLevel == 1)
                    thenpctospawn = 15572;
            else if(player.yakLevel == 2)
                thenpctospawn = 15573;

            NPCSpawning.spawnPet(player, thenpctospawn, player.absX + offsetX, player.absY + offsetY,
                    player.heightLevel, 0, true, false, true);
            if (!ignore) {
            }
        }
    ...
    public static boolean pickupPet(@NotNull Player player, int npcId, boolean item) {
        Pets pets = forNpc(npcId);
        player.modifierforpetdroprate=0;
        if (pets != null) {
