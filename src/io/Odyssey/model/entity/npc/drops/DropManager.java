...
public class DropManager {
    ...
    public final List<Integer> ordered = new ArrayList<>();
    ...
    static int[] godbookpages = { 3827,3828,3829,3830,3835,3836,3837,3838,3831,3832,3833,3834};
    ...
    static int[] revs = {7930, 7931, 7932, 7933, 7934, 7935, 7936, 7937, 7938, 7939, 7940, 7881};

    static int[] lowNPC = {100, 2216, 5816, 1680, 2241, 2218, 2005,2217, 3428, 3429, 970, 448, 414, 423, 446, 484, 268, 2834, 241, 2006, 2844, 484,
            2514, 6, 135, 208, 264, 7931, 7932, 2026, 957, 492, 957, 2916, 6918, 6914, 2098, 7268, 406, 1047, 421, 494, 435, 417, 437, 427, 411, 2834, 2841,
            2085, 7272, 7279, 7277, 2844, 975, 270, 7275, 7274, 7273, 247, 419, 3169, 6604, 2212, 3163, 3164, 3165, 7276, 3130, 2206, 2207, 2208, 3131, 3132,
            1673, 1672, 1677, 1674, 1675, 1676, 2463, 2840, 7933, 1610, 1611, 1612, 9026, 9031, 9027, 9028, 9029, 9030, 3135, 3139, 3134, 3140, 2209, 2211, 2210,
            2212, 2237, 2241, 3138, 3234, 2233, 2245, 3141, 3140, 3135, 3139, 3133, 3135, 3138, 3134, 3161, 2210, 2242, 2212, 3166, 3169, 3167, 3168,
            2244, 2234, 2237, 3133, 2235, 2243};

    static int[] medNPC = {2919, 5944, 1643, 415, 11, 7145, 7144, 498, 6606, 259, 7935, 7939, 7940, 7936, 7934, 7937, 7938, 1432, 7278, 7244,
            6342, 465, 273, 274, 499, 4005, 7250, 959, 8030, 8031, 9032, 9033, 9034, 6954};

    static int[] highNPC = {6611, 6609, 6615, 6610, 6593, 2054, 6619, 6618, 2265, 2266, 2267, 8781, 239, 5779, 494, 963, 2042, 2064, 2044, 5862,
            494, 2215, 5890, 319, 6766, 8027, 3162, 2205, 8028, 8615, 8620, 8621, 8622, 8026, 9021, 9022, 9023, 9021, 9022, 9023};
    ...
    public void handle(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        int godbookchance = 0;
        if(smallNpc){//add chances for everything here
            godbookchance = 250;
        } else if(mediumNpc){
            godbookchance = 200;
        } else if(largeNpc){
        ...
        if(Misc.random(godbookchance) < 10){
            Server.itemHandler.createGroundItem(player, Misc.random(godbookpages[godbookpages.length-1]), location.getX(), location.getY(), location.getZ(), 1, player.getIndex());
        }
        /**
         * Clue scrolls
         */
...
//        }
        ...
        if (Misc.random(crystalRandom) == 1) {
            ...
            if (player.hasFollower &&
              ...
              //  player.sendMessage("You notice a @blu@crystal key@bla@ on the floor.");
                Server.itemHandler.createGroundItem(player, specialItemId, location.getX(), location.getY(), location.getZ(), keyAmount, player.getIndex());
            }
        }
        //NpcDef npcDefinition = NpcDef.forId(npcId);
     ...
     //   int random = Misc.random(1200);
        ...
        if (IntStream.of(lowNPC).anyMatch(id -> id == npcId)) {
            BattlePass.small_npcs_bp_experience(player);
            player.getBattlePass().checkAlerts(player);
        }
        if (IntStream.of(medNPC).anyMatch(id -> id == npcId)) {
            BattlePass.medium_npcs_bp_experience(player);
            player.getBattlePass().checkAlerts(player);
        }
        if (IntStream.of(highNPC).anyMatch(id -> id == npcId)) {
            BattlePass.high_npcs_bp_experience(player);
            player.getBattlePass().checkAlerts(player);
        }
...
//            }
    ...
    public void create(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        group.ifPresent(g -> {
            ...
            for (GameItem item : drops) {//Here the items dropped from the NPC drop table.
                onDrop(player, item, npcId);
                Server.itemHandler.createGroundItem(player, item.getId(), location.getX(), location.getY(),
                        location.getZ(), item.getAmount(), player.getIndex());
            }
            handle(player, npc, location, repeats, npcId);//here do EVERYTHING after such as dropping clue scrolls , larrans keys  etc
        });
    }
...
//        }
    ...
    public void getDrops(Player player, int id) {//this is what populates the collection log with a given npc, just puts all RARE or VERY_RARE items int oit.
        Optional<TableGroup> group = groups.values().stream().filter(g -> g.getNpcIds().contains(id)).findFirst();
        group.ifPresent(g -> {
            List<GameItem> items = new ArrayList<>();
            for (TablePolicy policy : TablePolicy.POLICIES) {
           ...
           // }
            ...
            for (TablePolicy policy : TablePolicy.POLICIES) {
                ...
                if (table.isPresent()) {
                    ...
                    int rate2 = (int) Math.ceil((table.get().getAccessibility())) * table.get().size();
                  //  System.out.println("rateL: "+rate+" rate2: "+rate2);
...
//                    }
    ...
    public void onDrop(Player player, GameItem item, int npcId) {
        ...
        if (player.getItems().isWearingItem(21816) && player.absorption == true && item.getId() == 21820 && (IntStream.of(revs).anyMatch(id -> id == npcId))) {
            item.changeDrop(-1, 1);//funny way of making an npc for sure drop something .
        ...
        if (player.getItems().isWearingItem(21817) && player.absorption == true && item.getId() == 21820 && (IntStream.of(revs).anyMatch(id -> id == npcId))) {
            item.changeDrop(-1, 1);//funny way of making an npc for sure drop something .
            int amount = Misc.random(5);
            player.braceletIncrease(amount);
            player.getItems().setEquipment(21816, 1, 9,true);
            player.sendMessage("@red@The bracelet has collected some ether for you.");
        }
