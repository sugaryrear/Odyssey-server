...
import static io.Odyssey.model.Npcs.AHRIM_THE_BLIGHTED;
...
public class LoadSpell {
    ...
    public static ArrayList<Position> mole_burrow_points  = new ArrayList<>();


    static {
        mole_burrow_points.add( new Position(1742 ,5192,0));
        mole_burrow_points.add( new Position(1746,5172,0));
        mole_burrow_points.add( new Position(1774,5173,0));
        mole_burrow_points.add( new Position(1770,5200,0));
    ...
    public static void loadSpell(Player player, NPC npc) {
        ...
        switch (npc.getNpcId()) {
            ...
            case 7940: case 7881:
                if (npc.getHealth().getCurrentHealth() < npc.getHealth().getMaximumHealth() / 2) {
                   ...
                   // boolean distance = !player.goodDistance(npc.absX, npc.absY, player.getX(), player.getY(), 5);
                    if (randomHit <= 5 ) {
                        ...
                        npc.projectileId = -1;
                        npc.endGfx = -1;
                    } else if (randomHit > 5 && randomHit <= 10 ) {
                        npc.setAttackType(CombatType.MAGE);
                        npc.projectileId = 1415;
                        npc.endGfx = -1;
                    } else if (randomHit > 10 && randomHit <= 15) {
                ...
                break;
            case 11283://fumus attacks with shadow barrage
                // npc.setAttackType(CombatType.MAGE);
                npc.endGfx = 383;
                break;
            case 1605:
            ...
            case 520:
                ...
                if (random1223 <= 2) {
                  ...
                  npc.hitDelayTimer = 5;
                   npc.attackTimer = 8;
                } else if (random1223 > 2 && random1223 <= 5) {
                    npc.setAttackType(CombatType.MELEE);
                    npc.maxHit = 2;
                    npc.projectileId = -1;
                    npc.endGfx = -1;
            ...
            case 2043:
                ...
                //System.out.println("so it never gets there bnecause it keeps resetting?");
                npc.setAttackType(CombatType.MELEE);
                //npc.attackTimer = 9;
                //npc.hitDelayTimer = 6;
                npc.projectileId = -1;
            ...
            case 6619:
                ...
                break;
            case 6369:
boolean distancetagrithnana = player.goodDistance(npc.absX, npc.absY, player.getX(), player.getY(), 3);

if(distancetagrithnana){
    npc.setAttackType(CombatType.MELEE);
} else {
    npc.setAttackType(CombatType.MAGE);
    ...
    npc.endGfx = 131;
            ...
            case 6609:
                if (player != null) {
                    int randomHit3 = Misc.random(20);
                   // boolean distance = !player.goodDistance(npc.absX, npc.absY, player.getX(), player.getY(), 5);
                    if (randomHit3 < 15) {
                        npc.setAttackType(CombatType.MELEE);
                        npc.projectileId = -1;
                        npc.endGfx = -1;
                    } else if (randomHit3 >= 15 && randomHit3 < 20) {
                        npc.setAttackType(CombatType.MAGE);
...
//                break;
            ...
            case 239:
                ...
                if (random >= 60 && random < 65) {
                    npc.projectileId = 394; // green poison
                    npc.endGfx = 429;
                    npc.setAttackType(CombatType.DRAGON_FIRE);
                } else if (random >= 65 && random < 75) {
                    npc.projectileId = 395; // white - shock
                    npc.endGfx = 431;
                    npc.setAttackType(CombatType.DRAGON_FIRE);
                } else if (random >= 75 && random < 80) {
                    npc.projectileId = 396; // blue - freeze
                    npc.endGfx = 428;
                    npc.setAttackType(CombatType.DRAGON_FIRE);
                } else if (random >= 80 && distance <= 1) {
                    npc.projectileId = -1; // melee
                    npc.endGfx = -1;
                    npc.setAttackType(CombatType.MELEE);
                } else {
                    npc.projectileId = 393; // red - regular dragonfire
                    npc.endGfx = 430;
                    npc.setAttackType(CombatType.DRAGON_FIRE);
                }
                break;
