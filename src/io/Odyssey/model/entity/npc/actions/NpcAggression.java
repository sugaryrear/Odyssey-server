...
public class NpcAggression {
    ...
    private static final int[] REVS = {7931, 7932, 7933, 7934, 7935, 7936, 7937, 7938, 7939, 7940, 7881};
    ...
    public static void doAggression(NPC npc, NPCHandler npcHandler) {
        if ((isAggressive(npc) || npc.getCombatDefinition().isAggressive())
            ...
            for (Player player : PlayerHandler.players) {
                ...
                if (!Boundary.isIn(player, Boundary.GODWARS_BOSSROOMS)) {
                    if (god != null && player.getPosition().inGodwars() && player.getEquippedGodItems() != null
                            && player.getEquippedGodItems().contains(god)) {//good way to loop through an int array
                        continue;
                    }
                }
    ...
    private static int getAggressionDistance(NPC npc, Player player, int baseDistance) {
        ...
        if (Boundary.isIn(player, Boundary.SARACHNIS_LAIR)  && Boundary.isIn(npc, Boundary.SARACHNIS_LAIR)) {
            return 10;
        }
        if (Boundary.isIn(npc, Boundary.MIMIC_LAIR) || Boundary.isIn(npc, Boundary.GROTESQUE_LAIR) || Boundary.isIn(npc, Boundary.NexAngelOfDeath)) {
            return 10;
        }
