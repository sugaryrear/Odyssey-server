...
//			if (!c.getSlayer().getTask().isPresent() || !c.getSlayer().getTask().get().getPrimaryName().contains("black dragon")) {
...
//		}
		...
		if (Server.getEventHandler().isRunning(c, "hometele")) {
			Server.getEventHandler().stop(c, "hometele");
		}
		if (Server.getEventHandler().isRunning(c, "walkingtogateevent")) {
			Server.getEventHandler().stop(c, "walkingtogateevent");
		}
		c.nextChat = 0;
		c.dialogueOptions = 0;
...
//		}
	...
	//	System.out.println("yes: "+firstStepX+" and y: "+firstStepY);
