...
public class ClickNPC implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		/**
		 * Attack npc with magic, aka magiconnpc
		 **/
		case MAGE_NPC:
			if (c.morphed) {
				return;
		...
		if (n.getNpcId() == 11411) {
			...
			int distancenorth = 1;
			NpcDef npcDefinition = NpcDef.forId(c.npcType);
			if(npcDefinition != null)
				if(npcDefinition.getName() != null)
					if(npcDefinition.getName().toLowerCase().contains("banker") || npcDefinition.getName().toLowerCase().contains("reis")  || npcDefinition.getName().toLowerCase().contains("ket-yil") || npcDefinition.getName().toLowerCase().contains("ket-zuh")  || npcDefinition.getName().toLowerCase().contains("jade"))
					distancenorth = 2;
			if(n.getNpcId() == 687)
			distancenorth =3;
		if (n.distance(c.getPosition()) <= distancenorth) {
		...
		case SECOND_CLICK:
		 ...
		 distancenorth = 1;
			npcDefinition = NpcDef.forId(c.npcType);
			if(npcDefinition != null)
			...
			if(c.npcType  == 687)
				distancenorth =3;
			//System.out.println("npc: "+npcDefinition.getName());
			if (NPCHandler.npcs[c.npcClickIndex].distance(c.getPosition()) <= distancenorth) {
