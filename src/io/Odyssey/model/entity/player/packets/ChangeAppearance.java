...
//			return;
		...
		if (c.canChangeAppearance) {
			//System.out.println("sk: "+skinColour);

			if (skinColour == 9 && !c.getRights().isOrInherits(Right.REGULAR_DONATOR)) {//green color

				c.sendMessage("You must be at least a regular donator to use this skin color.");
				return;
			}
			if (skinColour == 11 && !c.getRights().isOrInherits(Right.EXTREME_DONOR)) {//purple color

				c.sendMessage("You must be at least an extreme donator to use this skin color.");
				return;
			}
			if (skinColour == 13 && !c.getRights().isOrInherits(Right.LEGENDARY_DONATOR)) {//rune color

				c.sendMessage("You must be at least a legendary donator to use this skin color.");
				return;
			}
			if (skinColour == 12 && !c.getRights().isOrInherits(Right.DIAMOND_CLUB)) {//lava

				c.sendMessage("You must be at least an diamond donator to use this skin color.");
				return;
			}

		//	if (skinColour != 9 && c.perksbought[2] == 0) {
				...
				if(!PlayerSave.playerExists(c.getDisplayName())){

					c.getModeSelection().openInterface();

				}

//			} else {
//				c.sendMessage("Only donators may choose green skin color.");
//
//			}
		}
	}
