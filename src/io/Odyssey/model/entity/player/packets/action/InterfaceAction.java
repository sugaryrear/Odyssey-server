...
public class InterfaceAction implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		player.debug(String.format("InterfaceAction id=%d, action=%d", id, action));
		...
		if (id >= 18144 && id < 18244) {
			for (int index = 0; index < 100; index++) {
				if (id == index + 18144) {
					...
					switch (action) {
					case 0:
						if (player.clan.isFounder(player.getDisplayName())) {
					...
					case 1:
						if (member.is(player)) {
							...
							if (player.clan.canKick(player.getDisplayName())) {
								player.clan.kickMember(member.getDisplayName());
							} else {
					...
					case 2:
						...
						if (clan != null) {
							if (clan.isRanked(member.getLoginName())) {
								...
								clan.banMember(Misc.formatPlayerName(member.getDisplayName()));
								player.getPA().setClanData();
								clan.save();
							}
						}
