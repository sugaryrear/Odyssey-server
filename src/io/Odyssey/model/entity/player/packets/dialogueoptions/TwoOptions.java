...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {  //see This handles option 1 if u scroll down ull see the section two
		...
		switch (c.dialogueAction) {
			case 4569:
				c.climbLadderTo(new Position(2505, 3641, 2));
				break;
			...
			case 44994:
				...
				break;
			case 834:
				c.getDH().sendDialogues(835, 954);
				break;
			case 837:
				c.getDH().sendDialogues(8388, 954);
				break;
			case 8390:
				c.getPA().closeAllWindows();
			...
			case 17560:
				...
				break;
			case 16672://staircase option going up
			...
			case 31559:
				c.getPA().closeAllWindows();
				if(c.getItems().playerHasItem(995, BountyHunter.revcavefee)) {
					c.getItems().deleteItem(995, BountyHunter.revcavefee);
					c.paidrevfee =true;
					c.sendMessage("@blu@You may now enter the Revenant Caves until you die.");
return;
				} else 	if (c.getBank().withdrawfrombank(41573, 995, BountyHunter.revcavefee)) {
				//	c.getItems().deleteItem(995, BountyHunter.revcavefee);
					c.paidrevfee =true;
					c.sendMessage("You may now enter the Revenant Caves until you die.");

					} else {
						c.sendMessage("@red@You need "+BountyHunter.revcavefee+" gp to enter the Revenant Cave.");
					}
			...
			case 21817:
				c.absorption = true;
				c.sendMessage("@pur@You have enabled Ether absorption.");
				c.getPA().closeAllWindows();
				break;
			case 8402:
				c.getDH().sendDialogues(8404, 8403);
				break;
            case 2034:
				c.getZulrahLostItems().retain();
				break;
			case 9414:
				c.getNightmareLostItems().retain();
				break;

			case 155_655://change yak name first option
				if(c.yakname != null){
					c.getDH().sendDialogues(155_656, 5816);
				} else {
					c.enterYakName();
				}
				...
				break;
			case 4925://gertrude
				if (c.getItems().getItemCount(1590, true) == 0 && c.petSummonId != 31014) {
					c.getItems().addItem(1590, 1);
					c.getDH().sendItemStatement("Verlrak hands you a dusty key.", 1590);
					c.nextChat = -1;

				} else {
					c.sendMessage("You already have a dusty key.");
					c.getPA().closeAllWindows();
				}

				break;
			case 7284://gertrude
				if (c.getItems().getItemCount(31014, true) == 0 && c.petSummonId != 31014) {
					c.getItems().addItem(31014, 1);
					c.getDH().sendItemStatement("Gertrude hands you pack yak.", 31014);
					c.nextChat = -1;
					c.sendMessage("if you die with the pack yak out you can get a new one from me.");
				} else {
					c.sendMessage("You already have a pack yak.");
					c.getPA().closeAllWindows();
				}

				break;
			case 15570:
				c.getDH().sendDialogues(15571, 1788);
				break;
			case 15581:
				c.getShops().openShop(210);
				break;
			case 11500:
				c.getDH().sendDialogues(11501, 1603);
				break;
			case 63364:
				c.getPA().closeAllWindows();
				if (!c.getItems().playerHasItem(995, 100_000)) {
					c.sendMessage("@red@You need 100,000 gp to go straight to the last wave.");

				} else {
					c.getItems().deleteItem(995, 100_000);
					c.getFightCave().create(1);

				}

				break;
			case 63363://fight cave regular
			...
			case 11981:
				...
				c.sendMessage("You will no longer automatically pickup currency.");
				c.getPA().closeAllWindows();
				break;
//			case 20377:
...
//				break;
			...
			case 20377:
				...
				c.getPA().closeAllWindows();
				break;
			case 10701://purchasing extra log perk
				if(c.prestigepoints < 30){
					c.sendMessage("@red@You need at least 30 prestige points to purchase this perk.");
					c.getPA().closeAllWindows();
				} else {
					c.sendMessage("@blu@You have purchased the extra log perk!");
					c.extralogperk = true;
					c.prestigepoints -=30;
			...
			case 10700:
				if(!c.extralogperk){
					c.getDH().sendOption2("Purchase Extra log perk (30 prestige points)", "Cancel");
					c.dialogueAction = 10701;
				} else {
					c.sendMessage("@red@You don't have any more perks to unlock.");
					c.getPA().closeAllWindows();
				}
			...
			case 10500://store
				...
				break;
			case 10000:
				var dailyTaskDate = c.DAILY_TASK_DATE;
				if(!c.getTaskMasterManager().checkfordate()){
					return;
				}
				c.getDH().sendDialogues(10001, 286);

				break;
			case 10001://task master dialog - skilling task
				c.getTaskMasterManager().giveTask(false,true,false);
			//	c.getPA().closeAllWindows();
				break;
			...
			case -500: // enter instance message
				...
				c.getPA().closeAllWindows();
				break;
			case 7206://jarvald to rellekka
				Sailing.startTravel(c,15);
			//	c.getPA().movePlayer(2620,3682,0);
			//	c.getPA().closeAllWindows();
				break;
			case 10177://waterbirth island dungeon
			...
			case 8929:
				...
				c.getPA().closeAllWindows();
				break;
			case 72066://jarvald to waterbirth island
				Sailing.startTravel(c,16);
//				c.getPA().movePlayer(2551,3755,0);
//				c.getPA().closeAllWindows();
				break;
			case 2584:
				c.getPA().closeAllWindows();
				c.getPA().startTeleport(1995, 3618, 0, "modern", false);

				break;
			case 180:
				c.getDH().sendDialogues(3197, 5422);
				break;
		case 1004:
			...
			case 13101:
				...
				c.getPA().startTeleport(c.specificteleport.tile.getX(), c.specificteleport.tile.getY(),  c.specificteleport.tile.getHeight(), "interface", false);
				c.getPA().closeAllWindows();
				if (c.getRights().contains(Right.REGULAR_DONATOR)) {

				} else {
					c.teleinterfaceLength = (int) Configuration.INTERFACE_TELE_LENGTH;
					c.teleinterfaceDelay = System.currentTimeMillis();
				}
...
//			break;
		...
		if (c.dialogueAction == 125) {
			if (c.getItems().playerHasItem(8851, 200) || SkillcapePerks.ATTACK.isWearing(c)
				...
				c.getWarriorsGuild().walkthrudoor(true);
//				//125
//				c.getPA().object(24309, 2487,3541, 0, 0);
//				c.getPA().object(24306, 2487 ,3540, 2, 0);
//				if(c.absX < 2847)
...
//				c.getWarriorsGuild().cycle();
				...
				c.getDH().sendNpcChat2("You need at least 200 warrior guild tokens.",
						"You can get some by operating the armour animator.", 2461, "Kamfreena");
				c.nextChat = 0;
			}
			c.dialogueAction = -1;
			c.teleAction = -1;
...
//		}
	...
	public static void handleOption2(Player c) {
		...
		switch (c.dialogueAction) {
			case 4569:
				c.climbLadderTo(new Position(2508, 3640, 0));
				break;
			case 8390:
				c.getDH().sendDialogues(8391, 954);
				break;
			case 180:
				c.getDH().sendDialogues(3199, 5422);
				break;

			case 2584:
				c.getPA().closeAllWindows();
				if (c.getItems().getItemCount(5509,false) == 0){
					c.getItems().addItem(5509, 1);
				} else {
					c.sendMessage("You already have a small pouch.");
				}
					break;

			case 10000://task master dialogue - reset task
				if(c.getItems().playerHasItem(995, TaskMasterManager.PRICE_TO_RESET_TASK)) {
					c.getItems().deleteItem(995, TaskMasterManager.PRICE_TO_RESET_TASK);
					c.getTaskMasterManager().resetTask();

				} else {
					c.sendMessage("You need "+TaskMasterManager.PRICE_TO_RESET_TASK+" gp to reset your current task.");
				}
				c.getPA().closeAllWindows();
			...
			case 21817:
				...
				break;
			case 8929:
				c.getPA().movePlayer(1918, 4367, 0);
				c.getPA().closeAllWindows();
				break;
			...
			case 10177://waterbirth island dungeon
				...
				c.getPA().closeAllWindows();
				break;
			case 16672://staircase option going down
						if(c.objectX == 2839 && c.objectY == 3537) { //warriors guild
							AgilityHandler.delayEmote(c, "CLIMB_UP", 2840, 3539, 0, 2);
						}
						if(c.objectX == 3204 && c.objectY == 3207) { //lumbridge

							AgilityHandler.delayEmote(c, "CLIMB_UP", 3205, 3209, 0, 2);
						}
						if(c.objectX == 3204 && c.objectY == 3229) { //lumbridge

							AgilityHandler.delayEmote(c, "CLIMB_UP", 3205, 3228, 0, 2);

						}
						//						if(c.objectX == 2839 && c.objectY == 3537) { //warriors guild
				//							c.getPlayerAssistant().movePlayer(2840, 3539, 0);
				//						}
				//						if(c.objectX == 3204 && c.objectY == 3207) { //lumbridge
				//							c.getPlayerAssistant().movePlayer(3205, 3209, 0);
				//						}
				//						if(c.objectX == 3204 && c.objectY == 3229) { //lumbridge
				//							c.getPlayerAssistant().movePlayer(3205, 3228, 2);
				//						}
						c.getPA().closeAllWindows();
	break;
			case 8402:
			...
			case 7206:
				...
				break;
			case 2034:
				c.getZulrahLostItems().whatitemsdoihave();
				break;
			case 9414:
				c.getNightmareLostItems().whatitemsdoihave();
				break;
			case 155_655://change yak name first option
					c.getDH().sendDialogues(155_657, 5816);
				break;

			case 7284:
				c.getDH().sendStatement("The pack yak will hold items for you. ","More features coming soon.","If you die with it out you can get another one from me.");
				c.nextChat = -1;
				break;

			case 11500:
				c.start(new KolodionDialogue(c));
				break;
			case 63363:

				c.getDH().sendOption2("Pay 100,000 gp", "Cancel");
				c.dialogueAction = 63364;
				break;
			case 63364:
			...
			case 11980:
				c.getDH().sendOption2("Stop picking up currency", "Automatically pick up currency");
				c.dialogueAction = 11981;
				break;
			case 11981:
				c.collectCoins = true;
				c.sendMessage("You will now automatically pickup currency.");
				c.getPA().closeAllWindows();
				break;
			case 20377:
				c.getPA().closeAllWindows();
			...
			case 10001://task master dialog - pvm task
				c.getTaskMasterManager().giveTask(false,false,true);
			//	c.getPA().closeAllWindows();
				break;
		case 1004:
			c.getPA().closeAllWindows();
