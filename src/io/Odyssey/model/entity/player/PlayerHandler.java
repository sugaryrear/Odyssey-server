...
public class PlayerHandler {
	...
	public static Optional<Player> getOptionalPlayerByCoords(int x, int y) {
		return getPlayers().stream().filter(Objects::nonNull).filter(client -> client.getX() == x &&  client.getY() == y).findFirst();
	}
	...
	public void process() {
		...
		onlinePlayers.forEach(player -> {//what really handles the process
			try {
				player.process();
			} catch (Exception e) {
	...
	public void sendObjectAnimation_cannon(GlobalObject object, int animation) {
		for (Player player : players) {
			if (player != null && player.distance(object.getPosition()) < 24
					&& player.getInstance() == object.getInstance() && player.getHeightLevel() == object.getPosition().getHeight()) {
				player.getPA().sendPlayerObjectAnimation(object.getPosition().getX(), object.getPosition().getY(), animation, object.getType(), -1);
			}
		}
	}
}
