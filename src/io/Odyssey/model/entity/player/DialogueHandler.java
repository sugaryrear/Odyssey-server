...
public class DialogueHandler {
	...
	public void sendDialogues(int dialogue, int npcId) {
		...
		if (dialogue == 0 && npcId == -1 && c.getCurrentCombination().isPresent()) {// npcId == -1  very important
		...
		switch (dialogue) {
			...
			case 1018:
				...
				break;
			case 1019:
				if (c.absX == 3267 || c.absX == 3268) {
					sendNpcChat1("You must pay a toll of 10 gold coins to pass.",
							c.talkingNpc, "Border Guard");
					c.nextChat = 1020;
				} else {
					sendNpcChat1("You need to be closer to the gate to use it.", c.talkingNpc, "Border Guard");
					c.nextChat = 0;
				}
				break;
			case 1020:
			...
			case 4642://shantay
				sendNpcChat("In the desert you will take damage every minute.", "(Every 2m 30s with Desert Robes)");
				c.nextChat = -1;
				break;
			...
			case 17559:
				sendOption2("Dig down anyway", "Cancel");
				c.dialogueAction = 17560;
				break;
			case 3195:
				sendNpcChat("Say you got any of those mole skins or claws?");
				c.nextChat = 3196;
				break;
			...
			case 3196:
				...
				break;
			case 3197:
				int MOLE_SKIN = c.getItems().getItemAmount(7418), MOLE_CLAW = c.getItems().getItemAmount(7416);
				...
				if (MOLE_SKIN > 0) {
					...
					c.talkingNpc = 5422;
					sendNpcChat("You don't have any mole skins.");
					c.nextChat = 0;
				}
				break;
			case 3198:
				MOLE_SKIN = c.getItems().getItemAmount(7418);
				c.getItems().deleteItem(7418, MOLE_SKIN);
				c.getItems().addItemUnderAnyCircumstance(7413, MOLE_SKIN);
				sendNpcChat("Here you go");
				c.nextChat = 0;
				break;
			case 3199:
				MOLE_CLAW = c.getItems().getItemAmount(7416);
				if (MOLE_CLAW > 0) {
					sendPlayerChat2("I would like to exchange my " + MOLE_CLAW + " mole claws,", "for bird nests.");
					c.nextChat = 3200;
				} else {
					c.talkingNpc = 5422;
					sendNpcChat("You don't have any mole claws.");
					c.nextChat = 0;
				}
				break;
			...
			case 3200:
				MOLE_CLAW = c.getItems().getItemAmount(7416);
				c.getItems().deleteItem(7416, MOLE_CLAW);
				c.getItems().addItemUnderAnyCircumstance(7413, MOLE_CLAW);
				c.talkingNpc = 5422;
				sendNpcChat("Here you go");
				c.nextChat = 0;
				break;
			case 7653:
				sendNpcChat("This area is only for those who have a", "black dragon task.");
				c.nextChat = -1;
				break;

			case 7672:
				sendNpcChat("This area is only for those who have a", "blue dragon task.");
				c.nextChat = -1;
				break;
			case 1337:
				sendNpcChat("Sup");
			...
			case 944:
				sendNpcChat("Get ready! Here comes Tztok-Jad!");
				c.nextChat = -1;
				break;

			case 31556:
				sendStatement("To enter the revenant caves you must pay "+ Misc.insertCommas(BountyHunter.revcavefee)+" gp.");
				c.nextChat = 31557;
				break;
			case 31557:
				sendStatement("If you die inside the caves you must pay the fee again.");
				c.nextChat = 31558;
				break;
			case 31558://confirm 100k
				sendOption2("Pay ("+Misc.insertCommas(BountyHunter.revcavefee)+" gp (can be taken from bank)", "Cancel");
				c.dialogueAction = 31559;
				break;
			case 5937://jarvald to waterbirth island
				sendOption2("Travel to Waterbirth Island", "Cancel");
				c.dialogueAction = 72066;
				break;

			case 7205://jarvald to rellekka
				sendOption2("Travel to Rellekka", "Cancel");
				c.dialogueAction = 7206;
				break;
			case 8402://askelaaden
				sendOption2("How do I get past the door inside?", "Cancel");
				c.dialogueAction = 8402;
				break;
			case 8404:
				sendNpcChat("What you need to do is drop something heavy", "on one of the tiles then stand on the other", "Try this!");
				c.nextChat = 8406;
				break;
			case 8406:
				if (c.getItems().getItemCount(3695,false) == 0){
					c.getItems().addItem(3695, 1);
					c.getDH().sendItemStatement("You receive a pet rock.", 3695);
					return;
			} else {
					c.sendMessage("You already have a pet rock.");
					c.getPA().closeAllWindows();
				}

				break;
			case 2033://purchasing items back zulrah
				sendOption2("Purchase items back ("+ ZulrahLostItems.price+" gp)?" ,"What items do I have?");
				c.dialogueAction = 2034;
				break;
			case 9414://purchasing items back shura / nightmare
				sendOption2("Purchase items back ("+ NightmareLostItems.price+" gp)?" ,"What items do I have?");
				c.dialogueAction = 9414;
				break;
			case 155_656:
				sendNpcChat("My name is "+c.yakname+"!");
				c.nextChat = -1;
				break;
			case 155_657:
				long milliseconds = (long) c.yakplayTime * 600;
				long days = TimeUnit.MILLISECONDS.toDays(milliseconds);
				long hours = TimeUnit.MILLISECONDS.toHours(milliseconds - TimeUnit.DAYS.toMillis(days));
				String time = "I am "+days + " days, " + hours + " hrs old!";
				sendNpcChat(time);
				//in will be just get the milliseconds of 1 hour or some shit
				c.nextChat = -1;
				break;
			case 4407://ava in draynor manor
				if (c.getLevelForXP(c.playerXP[4]) < 50) {
					c.sendMessage("Ava isn't interested in talking to you.");
					return;
				}
				sendNpcChat("My accumulators have a 72% chance of recovering","your arrows!");
				c.nextChat = -1;
				break;

			case 15571:
				sendNpcChat2("Lundail at Mage bank, Tzhaar-Mej-Roh in Mor-Ui-Rek,", "Mage store at Void knight outpost", 1160, "Wizard");
				c.nextChat = 15572;
				break;
			case 15572:
				sendNpcChat2("Magic shop at the magic guild","", 1160, "Wizard");
				c.nextChat = -1;
				break;
			/**
			 * Mage Arena - Kolodion
			...
			case 150:
				if(c.maRound < 2) {
					...
					sendNpcChat("You beat my ass pretty good back there! You can now",
							"use the whirlpool and also cast god spells.");
					c.nextChat = 11500;
				}
				break;
			case 11500:

				sendOption2("How many times have I casted each god spell?" ,"Mage Arena II");
				c.dialogueAction = 11500;
				break;
			case 11501:
				sendNpcChat("Saradomin Strike: "+c.saradominStrikeCasts+" times",
						"Claws of Guthix: "+c.clawsOfGuthixCasts+" times",
						"Flames of Zamorak: "+c.flamesOfZamorakCasts+" times");
				c.nextChat = -1;
				break;

			case 151:
				sendNpcChat3("I am the great Kolodian, master of battle magic, and",
						"this is my battle arena. Top wizards travel from all over", "Runescape to fight here.",
						1603, "Kolodion");
				c.nextChat = 152;
				break;
			case 152:
			...
			case 153:
				...
				break;
			case 154:
				sendNpcChat3("My arena is open to any high level wizard, but this is",
			...
			case 159:
				sendNpcChat4("Remember, traveller - in my arena, hand-to-hand",
						...
						"amongst the most powerful in all of Runescape!", 1603, "Kolodion");
				c.nextChat = 160;
				break;
			case 160:
			...
			case 7284://gertrude
				sendOption2("I need a Pack Yak", "What is this?");
				c.dialogueAction = 7284;
				break;
			...
			case 62357://switching back to normal prayer
				...
				break;
			case 10700://prestiging
				sendOption2("Unlock prestige ability (woodcutting)", "prestige shop");
				c.dialogueAction = 10700;
				break;
			case 10000://task master dialogue:
				sendOption2("Receive task", "reset task ("+ TaskMasterManager.PRICE_TO_RESET_TASK+" gp)");
				c.dialogueAction = 10000;
				break;

				case 10001://task master dialogus - select which type
				sendOption2("Skilling Task", "PvM Task");
				c.dialogueAction = 10001;
				break;
			case 10200://vote button
				c.getDH().sendOption2("Vote (opens website)", "Vote Interface");
				c.dialogueAction = 10200;
				break;
			case 10500://donate button
				c.getDH().sendOption2("Donate (opens website)", "cancel");
		...
		case 179:
			sendPlayerChat1("I need an assignment.");
			if (c.getSlayer().getTask().isPresent()) {
				c.getDH().sendDialogues(3305, c.talkingNpc);
			} else {
				c.nextChat = 180;
			}
		break;
		...
		case 1002:
			...
			break;
		case 180:
			//here is where you deviate depending on what master you are talking to
			if(c.talkingNpc == 401) {//turael
				SlayerMaster.get(401);
				c.getSlayer().createNewTask(Slayer.EASY_TASK_NPC_ID, false);
			} else if (c.talkingNpc == 402){//mazchna
				SlayerMaster.get(402);
				c.getSlayer().createNewTask(Slayer.MEDIUM_TASK_NPC_ID, false);
			} else if (c.talkingNpc == 403){//vannaka
				SlayerMaster.get(403);
				c.getSlayer().createNewTask(Slayer.VANNAKA_TASK_NPC_ID, false);
			} else if (c.talkingNpc == 404){//chaeldar
				SlayerMaster.get(404);
				c.getSlayer().createNewTask(Slayer.CHAELDAR_TASK_NPC_ID, false);
			} else if (c.talkingNpc == 405){//duradel
				SlayerMaster.get(405);
				c.getSlayer().createNewTask(Slayer.HARD_TASK_NPC_ID, false);
			}
			else if (c.talkingNpc == 6797){//nieve
		...
		case 706:
			...
			//	c.getQuestTab().updateInformationTab();
				...
				sendNpcChat("Come back with an anti-dragon shield, draconic visage and 5M Gold!");
			}
			c.nextChat = -1;
			break;
		...
		case 66:
			...
			break;
		case 832:
			sendNpcChat("They're coming out of the walls!!");
			c.nextChat = 833;
			break;
			case 833:
				sendOption2("How do I get down there?", "Nevermind");
				c.dialogueAction = 834;
				break;
			case 835:
				sendNpcChat("Hmm you'll need some sort of rope maybe?");
				c.nextChat = 836;
				break;
			case 836:
				sendOption2("...you don't happen to have a spare one do you?", "Nevermind");
				c.dialogueAction = 837;
				break;
			case 8388:
				sendNpcChat("50 gp take it or leave it!");
				c.nextChat = 839;
				break;
			case 8391:
				sendNpcChat("Do you see a general store nearby?");
				c.nextChat = 839;
				break;
			case 839:
				sendOption2("Pay (50 gp)", "But it's 18 gp in the general store...");
				c.dialogueAction = 8390;
				break;
			case 83555:
				sendNpcChat("By the way you will need another rope", "to get into the Kalphite Queens lair.");
				c.nextChat = -1;
				break;
		case 67:
		...
		case 3300:
			...
			sendNpcChat("'Ello, and what are you after then?");
			c.nextChat = 3301;
			break;
		case 11155:
		...
		case 3301:
			sendOption4("I'd like to see the slayer interface.", "I am in need of a slayer assignment.",
					"Could you tell me where I can find my current task?", "Cancel my task");
		...
		/* Bank Settings **/
		case 1013:
			c.getDH().sendNpcChat("Good day. How may I help you?");
			c.nextChat = 1014;
			break;
		...
		case 1015:
			...
			break;
		case 1016:
			c.getDH().sendNpcChat2("This is the bank of Runescape", "We have many branches in many towns.",
					c.talkingNpc, "Banker");
			c.nextChat = 0;
			break;
		/*
...
//					c.talkingNpc, "Emblem Trader");
		...
		case 480:
			...
			break;
			case 13100:
				sendOption2("Teleport to "+c.specificteleport.text, "Cancel");
				c.dialogueAction = 13101;
				break;
		case 481:
		...
		case 631:
			sendStatement("Pay "+Misc.insertCommas(Configuration.priceofresourcearea)+" gp?");
			c.nextChat = 632;
			break;
...
//			c.dialogueAction = 128;
				c.getDH().sendOption2("Regular (63 waves)", "Go straight to last wave (100,000 gp)");
				c.dialogueAction = 63363;
				break;
		...
		case 7110:
			c.getDH().sendOption3( "Replay Tutorial",
					"Got any quests?","Referral");
			c.dialogueAction = 71111;
			break;


			case 71112:
				sendNpcChat("Hmm... let me check.");
				c.nextChat = 71113;
				break;
			case 71118:
	sendNpcChat("Not at the moment.");
			c.nextChat = -1;
				break;
			case 71113:

				c.getDH().sendOption3("Horror From the Deep", "Recipe for Disaster",
						"Monkey Madness");
				c.dialogueAction = 71115;
				break;
			case 71116:
				sendNpcChat("I heard of a disturbance at the lighthouse near Rellekka","Go check it out.");
				c.nextChat = -1;
				break;
			case 71126:
				sendNpcChat("The cook in Lumbridge castle needs some help.","Go check it out.");
				c.nextChat = -1;
				break;
			case 71136:
				sendNpcChat("King Nardone Shareen at the Tree Gnome Stronghold","needs some help.");
				c.nextChat = -1;
				break;
		case 711:
