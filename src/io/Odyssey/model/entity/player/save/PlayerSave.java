...
public class PlayerSave {
    ...
    public static String getSaveDirectory_FAVS() {
        return Server.getSaveDirectory() + "/favorites/";
    }
    public static String getSaveDirectory_UNLOCKEDREGIONS() {
        return Server.getSaveDirectory() + "/unlockedregions/";
    }
    public static String getSaveDirectory_gwdkc() {
        return Server.getSaveDirectory() + "/gwd/";
    }
    ...
    private static void loadSettings(Player p, String location1) {
       ...
       private static void loadSettings_unlockedregions(Player p, String location1) {
        File location = new File(location1);
        if (location.exists()) {
            loadSaved_unlockedregions(p);
            return;
        }
        try {
            location.createNewFile();
  ...
  //  private static void loadSettings_gwdkc(Player p, String location1) {
//        File location = new File(location1);
//        if (location.exists()) {
//            loadSaved_gwdkc(p);
//            return;
...
//        }
//    }
    private static void loadSaved(Player p){
        try {
            fromJson(p);
    ...
    private static void loadSaved_unlockedregions(Player p){
        try {
            fromJson_unlockedregions(p);
        } catch (FileNotFoundException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static void fromJson(Player p) throws FileNotFoundException {
        Gson gson = new Gson();
        SpecificTeleport[] s = gson.fromJson(new FileReader(getSaveDirectory_FAVS() + p.getLoginName() + ".json"), SpecificTeleport[].class);
        for (SpecificTeleport sp : s) {
            if (sp == null)
                continue;
           // p.setnewtelefavs(sp);
            p.getnewfavs().add(new SpecificTeleport(sp.button,sp.tile,sp.text,sp.description,sp.favorited,sp.favoritebutton,sp.regionunlock));
        }

    }
    private static void fromJson_unlockedregions(Player p) throws FileNotFoundException {

        Gson gson = new Gson();

        try (FileReader reader = new FileReader(getSaveDirectory_UNLOCKEDREGIONS() + p.getLoginName() + ".json")) {
            // Define the TypeToken for the map
            Type mapType = new TypeToken<Map<Integer, Boolean>>() {}.getType();

            // Parse the JSON data into a Map<Integer, Boolean>
            p.regionsunlocked =  gson.fromJson(reader, mapType);
        } catch (IOException e) {
            e.printStackTrace();
          //  return null;
        }
//System.out.println("r: "+p.regionsunlocked.size());
    }
//    private static void fromJson_godwarskc(Player p) throws FileNotFoundException {
//
//        Gson gson = new Gson();
//
//        try (FileReader reader = new FileReader(getSaveDirectory_gwdkc() + p.getLoginName() + ".json")) {
//            // Define the TypeToken for the map
//            Type mapType = new TypeToken<Map<God, Integer>>() {}.getType();
//
//            // Parse the JSON data into a Map<Integer, Boolean>
//            p.getGodwars().killcount =  gson.fromJson(reader, mapType);
//        } catch (IOException e) {
//            e.printStackTrace();
//            //  return null;
//        }
...
//    }
    ...
    public static LoadGameResult loadGame(Player p, String playerName, String playerPass, boolean passedCaptcha) {
        ...
        loadSettings(p,getSaveDirectory_FAVS() + playerName + ".json");
        loadSettings_unlockedregions(p,getSaveDirectory_UNLOCKEDREGIONS() + playerName + ".json");
       // loadSettings_gwdkc(p,getSaveDirectory_gwdkc() + playerName + ".json");
                ...
                if (spot > -1) {
                    ...
                    switch (ReadMode) {
                        ...
                        case 2:
                            if (token.equals("character-height")) {
                            ...
                            } else if (token.equals("discord-user")) {
                                p.setDiscordUser(Long.parseLong(token2));
                            } else if (token.equals("discord-tag")) {
                                p.setDiscordTag(token2);
                            } else if (token.equals("yakname")) {
                                p.yakname = token2;
                            } else if (token.equals("display-name")) {
                                p.setDisplayName(token2);// = token2;
                            } else if (token.equals("slayerpartner-name")) {
                                p.partnername = token2;
                            } else if (token.equals("curseprayers")) {
                                p.usingcurseprayers = Boolean.parseBoolean(token2);
                            } else if (token.equals("unlockedallteles")) {
                                p.unlockedallteles = Boolean.parseBoolean(token2);
                            } else if (token.equals("firsttimetut")) {
                                p.firsttimetutorial = Boolean.parseBoolean(token2);
                            ...
                            } else if (token.equals("nightmare-items")) {
                                if (token3.length > 1) {
                                    for (int i = 0; i < token3.length; i += 2) {
                                        int itemId = Integer.parseInt(token3[i]);
                                        int itemAmount = Integer.parseInt(token3[i + 1]);
                                        p.getNightmareLostItems().add(new GameItem(itemId, itemAmount));
                                    }
                                }
                                ...
                                try {
                                    ...
                                    type = Enum.valueOf(Tasks.class, token2);
                                } catch (NullPointerException | IllegalArgumentException e) {
                                    e.printStackTrace();
                                    logger.error("Error while loading task {}, type={}", playerName, token2, e);
                                    break;
                                }
                               // Tasks mode = Tasks.forType(type);
                                p.TASK = type;
                            } else if (token.equals("taskAmount")) {
                                p.TASK_AMOUNT = (Integer.parseInt(token2));
                            } else if (token.equals("taskCompletionAmount")) {
                                p.TASK_COMPLETE_AMOUNT = (Integer.parseInt(token2));
                            } else if (token.equals("totalTasksCompleted")) {
                                p.TASKS_COMPLETED = (Integer.parseInt(token2));
                            } else if (token.equals("canClaimTaskReward")) {
                                p.CAN_CLAIM_TASK_REWARD = Boolean.parseBoolean(token2);
                            } else if (token.equals("completedDailyTask")) {
                                p.COMPLETED_DAILY_TASK = Boolean.parseBoolean(token2);
                            } else if (token.equals("dailytaskdate")) {
                                p.DAILY_TASK_DATE = (Integer.parseInt(token2));
                            ...
                            } else if (token.equals("notification_broadcast")) {
                                p.notification_broadcast = Boolean.parseBoolean(token2);
                            } else if (token.equals("collectCoins")) {
                                p.collectCoins = Boolean.parseBoolean(token2);
                            } else if (token.equals("gwdentrance")) {
                                p.learnedGWDentrance = Boolean.parseBoolean(token2);
                            ...
                            } else if (token.equals("puremode")) {
                                p.puremode = Boolean.parseBoolean(token2);
                            } else if (token.equals("paidrevfee")) {
                                p.paidrevfee = Boolean.parseBoolean(token2);
                            }else if (token.equals("boughtBP")) {
                                p.boughtBP = Boolean.parseBoolean(token2);
                            ...
                            } else if (token.equals("questscompleted")) {
                                p.questsCompleted = Integer.parseInt(token2);
                            } else if (token.equals("questpoints")) {
                                p.totalqp = Integer.parseInt(token2);
                            ...
                            } else if (token.equals("gwdkc")) {
                                for (int i = 0; i < p.gwdkc.length; i++) p.gwdkc[i] = Integer.parseInt(token3[i]);
                            } else if (token.equals("firstsaradomingwd")) {
                                p.firstropesaradomingwd = Boolean.parseBoolean(token2);
                            } else if (token.equals("secondsaradomingwd")) {
                                p.secondropesaradomingwd = Boolean.parseBoolean(token2);
                            } else if (token.equals("hasunlockedfrozendoor")) {
                                p.hasunlockedfrozendoor = Boolean.parseBoolean(token2);
                            ...
                            } else if (token.equals("ddScroll")) {
                                p.ddScroll = Boolean.parseBoolean(token2);
                            } else if (token.equals("ddScrollTime")) {
                                p.ddScrollTicks = Long.parseLong(token2);
                            ...
                            } else if (token.equals("holy-book")) {
                                for (int i = 0; i < p.holybook.length; i++) p.holybook[i] = Integer.parseInt(token3[i]);
                            } else if (token.equals("balance-book")) {
                                for (int i = 0; i < p.balancebook.length; i++) p.balancebook[i] = Integer.parseInt(token3[i]);
                            } else if (token.equals("unholy-book")) {
                                for (int i = 0; i < p.unholybook.length; i++) p.unholybook[i] = Integer.parseInt(token3[i]);
                            ...
                            } else if (token.equals("extralogperk")) {
                                p.extralogperk = Boolean.parseBoolean(token2);
                            ...
                            } else if (token.equals("battlePage")) {
                                p.battlePage = Integer.parseInt(token2);
                            } else if (token.equals("currentTier")) {
                                p.currentTier = Integer.parseInt(token2);
                            } else if (token.equals("currentTierXP")) {
                                p.currentTierXP = Integer.parseInt(token2);
                            ...
                            } else if (token.equals("hometele-length")) {
                                p.homeTeleportDelay = System.currentTimeMillis();
                                p.homeTeleportLength = Integer.parseInt(token2);
                            } else if (token.equals("randomevent-length")) {
                                p.randomeventDelay = System.currentTimeMillis();
                                p.randomeventLength = Integer.parseInt(token2);
                            } else if (token.equals("interfacetele-length")) {
                                p.teleinterfaceDelay = System.currentTimeMillis();
                                p.teleinterfaceLength = Integer.parseInt(token2);
                            } else if (token.equals("infrunenergy-length")) {
                                p.infrunenergyDelay = System.currentTimeMillis();
                                p.infrunenergyLength = Integer.parseInt(token2);
                            ...
                            } else if (token.equals("slayer-task-origin")) {
                                p.getSlayer().setTaskAmountorig(Integer.parseInt(token2));
    ...
    private static void updateFile(Player p) throws IOException {
        new PrintWriter(getSaveDirectory_FAVS() + p.getLoginName() + ".json").close();
        //   p.getnewfavs().clear();
        try (PrintWriter out = new PrintWriter(new FileWriter(getSaveDirectory_FAVS() + p.getLoginName() + ".json"))) {
            Gson gson = new Gson();
            String jsonString = gson.toJson(p.getnewfavs());
            out.write(jsonString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    private static void updateFile_regions(Player p) throws IOException {
        new PrintWriter(getSaveDirectory_UNLOCKEDREGIONS() + p.getLoginName() + ".json").close();
        //   p.getnewfavs().clear();
        try (PrintWriter out = new PrintWriter(new FileWriter(getSaveDirectory_UNLOCKEDREGIONS() + p.getLoginName() + ".json"))) {
            Gson gson = new Gson();
            String jsonString = gson.toJson(p.regionsunlocked);
            out.write(jsonString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
//    private static void updateFile_gwd(Player p) throws IOException {
//        new PrintWriter(getSaveDirectory_gwdkc() + p.getLoginName() + ".json").close();
//        //   p.getnewfavs().clear();
//        try (PrintWriter out = new PrintWriter(new FileWriter(getSaveDirectory_gwdkc() + p.getLoginName() + ".json"))) {
//            Gson gson = new Gson();
//            if(p.getGodwars().killcount == null)
//                p.getGodwars().killcount = new HashMap<>();
//            String jsonString = gson.toJson(p.getGodwars().killcount);
//            out.write(jsonString);
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
    private static void saveMapToTextFile(Map<Integer, Boolean> map, String filePath) {
        // Import necessary libraries


        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            for (Map.Entry<Integer, Boolean> entry : map.entrySet()) {
                // Convert each entry into a text format, for example: "1=true"
...
//            throw new RuntimeException(e);
//        }
       // saveMapToTextFile(p.regionsunlocked,getSaveDirectory_UNLOCKEDREGIONS() + p.getLoginName() + ".txt");
        int tbTime = (int) (p.teleBlockStartMillis - System.currentTimeMillis() + p.teleBlockLength);
        if (tbTime > 300000 || tbTime < 0) {
            tbTime = 0;
        }
        int hometeleTime = (int) (p.homeTeleportDelay - System.currentTimeMillis() + p.homeTeleportLength);
        if (hometeleTime > Configuration.HOMETELE_LENGTH || hometeleTime < 0) {
            hometeleTime = 0;
        }
        int randomeventTime = (int) (p.randomeventDelay - System.currentTimeMillis() + p.randomeventLength);
        if (hometeleTime > Configuration.RANDOM_EVENT_LENGTH || randomeventTime < 0) {
            randomeventTime = 0;
        }
        int interfaceteleTime = (int) (p.teleinterfaceDelay - System.currentTimeMillis() + p.teleinterfaceLength);
        if (interfaceteleTime > Configuration.INTERFACE_TELE_LENGTH || interfaceteleTime < 0) {
            interfaceteleTime = 0;
        }
        int infrunenergyTime = (int) (p.infrunenergyDelay - System.currentTimeMillis() + p.infrunenergyLength);
        if (infrunenergyTime > Configuration.INF_RUN_ENERGY_LENGTH || infrunenergyTime < 0) {
            infrunenergyTime = 0;
        }
        try {
            p.getFarming().save();
        ...
        try {
            ...
            characterfile.newLine();
            characterfile.write("slayerpartner-name = " + p.partnername);
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("run-energy  = ");
            characterfile.write(Integer.toString(p.getRunEnergy() ));
            characterfile.newLine();
            ...
            characterfile.newLine();
            if (p.getDiscordUser() > 0) {
                characterfile.write("discord-user = " + p.getDiscordUser());
                characterfile.newLine();
            }
            characterfile.write("discord-tag = " + p.getDiscordTag());
            characterfile.newLine();
            characterfile.write("yakname = " + p.yakname);
            characterfile.newLine();
            ...
            characterfile.write("curseprayers = " + p.usingcurseprayers);
            characterfile.newLine();
            characterfile.write("unlockedallteles = " + p.unlockedallteles);
            characterfile.newLine();
            characterfile.write("firsttimetut = " + p.firsttimetutorial);
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("nightmare-items = ");
            for (GameItem item : p.getNightmareLostItems()) {
                if (item == null) {
                    continue;
                }
                characterfile.write(item.getId() + "\t" + item.getAmount() + "\t");
            ...
            if (p.TASK != null) {
                characterfile.write("task = " + p.TASK.name());
                characterfile.newLine();
            } else {
                characterfile.write("task = " + Tasks.NONE);
                characterfile.newLine();
            }

            characterfile.write("taskAmount = " + p.TASK_AMOUNT);
            characterfile.newLine();
            characterfile.write("taskCompletionAmount = " + p.TASK_COMPLETE_AMOUNT);
            characterfile.newLine();
            characterfile.write("totalTasksCompleted = " + p.TASKS_COMPLETED);
            characterfile.newLine();
            characterfile.write("dailytaskdate = " + p.DAILY_TASK_DATE);
            characterfile.newLine();
            characterfile.write("canClaimTaskReward = " + p.CAN_CLAIM_TASK_REWARD);
            characterfile.newLine();
            characterfile.write("completedDailyTask = " + p.COMPLETED_DAILY_TASK);
            characterfile.newLine();
            // EventCalendar
            ...
            characterfile.newLine();
            characterfile.write("barrows-completed = " + p.getBarrows().isCompleted());
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("notification_broadcast = " + p.notification_broadcast);
            characterfile.newLine();
            characterfile.write("puremode = " + p.puremode);
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("paidrevfee = " + p.paidrevfee);
            characterfile.newLine();
            characterfile.write("boughtBP = ", 0, 11);
            characterfile.write(Boolean.toString(p.boughtBP), 0, Boolean.toString(p.boughtBP).length());
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("questscompleted = " + p.questsCompleted);
            characterfile.newLine();
            characterfile.write("questpoints = " + p.totalqp);
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("ddScroll = ");
            characterfile.write(Boolean.toString(p.ddScroll));
            characterfile.newLine();
            characterfile.write("ddScrollTime = ");
            characterfile.write(Long.toString(p.ddScrollTicks));
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("holy-book = " + p.holybook[0] + "\t" + p.holybook[1] + "\t" + p.holybook[2] + "\t" + p.holybook[3]);
            characterfile.newLine();
            characterfile.write("balance-book = " + p.balancebook[0] + "\t" + p.balancebook[1] + "\t" + p.balancebook[2] + "\t" + p.balancebook[3]);
            characterfile.newLine();
            characterfile.write("unholy-book = " + p.unholybook[0] + "\t" + p.unholybook[1] + "\t" + p.unholybook[2] + "\t" + p.unholybook[3]);
            characterfile.newLine();
            characterfile.write("counters = ");
            for (int i = 0; i < p.counters.length; i++) characterfile.write("" + p.counters[i] + ((i == p.counters.length - 1) ? "" : "\t"));
            characterfile.newLine();
            characterfile.write("gwdkc = ");
            for (int i = 0; i < p.gwdkc.length; i++) characterfile.write("" + p.gwdkc[i] + ((i == p.gwdkc.length - 1) ? "" : "\t"));
            characterfile.newLine();
            characterfile.write("firstsaradomingwd = ");
            characterfile.write(Boolean.toString(p.firstropesaradomingwd));
            characterfile.newLine();
            characterfile.write("secondsaradomingwd = ");
            characterfile.write(Boolean.toString(p.secondropesaradomingwd));
            characterfile.newLine();
            characterfile.write("hasunlockedfrozendoor = ");
            characterfile.write(Boolean.toString(p.hasunlockedfrozendoor));
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("gwdentrance = " + p.learnedGWDentrance);
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("prestigepoints = " + p.prestigepoints);
            characterfile.newLine();
            characterfile.write("extralogperk = ");
            characterfile.write(Boolean.toString(p.extralogperk));
            characterfile.newLine();
            characterfile.write("battlePage = ", 0, 13);
            characterfile.write(Integer.toString(p.battlePage), 0, Integer.toString(p.battlePage).length());
            characterfile.newLine();
            characterfile.write("currentTier = ", 0, 14);
            characterfile.write(Integer.toString(p.currentTier), 0, Integer.toString(p.currentTier).length());
            characterfile.newLine();
            characterfile.write("currentTierXP = ", 0, 16);
            characterfile.write(Integer.toString(p.currentTierXP), 0, Integer.toString(p.currentTierXP).length());
            characterfile.newLine();
            ...
            characterfile.newLine();
            characterfile.write("hometele-length = ", 0, 18);
            characterfile.write(Integer.toString(hometeleTime), 0, Integer.toString(hometeleTime).length());
            characterfile.newLine();
            characterfile.write("randomevent-length = ", 0, 21);
            characterfile.write(Integer.toString(randomeventTime), 0, Integer.toString(randomeventTime).length());
            characterfile.newLine();
            characterfile.write("interfacetele-length = " + interfaceteleTime);
            characterfile.newLine();
            characterfile.write("infrunenergy-length = " +infrunenergyTime);
            characterfile.newLine();
            //Varrock
...
//correct way to do it
            if (p.getSlayer().getTask().isPresent()) {
                ...
                characterfile.newLine();
                characterfile.write("slayer-task-origin = " + p.getSlayer().getTaskAmountorig());
                characterfile.newLine();
            }
            ...
            /* pakyak */
            characterfile.write("[DEPOSITB]", 0, 10);
            characterfile.newLine();
            for (int i = 0; i < p.pakyakitems.size(); i++) {
                if (p.pakyakitems.get(i).getId() > 0) {
                    characterfile.write("dbag-item = ", 0, 12);
                    characterfile.write(Integer.toString(i), 0, Integer.toString(i).length());
                    characterfile.write("\t", 0, 1);
